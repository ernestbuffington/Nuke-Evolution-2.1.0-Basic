...
global $admin_file, $ab_config, $db, $bgcolor2, $bgcolor1;
...
if (is_admin()) {
    ...
    $perpage = 100;
    $totalselected = $db->sql_unumrows("SELECT `ip_lo` FROM `"._SENTINEL_EXCLUDED_RANGES_TABLE."`");
    if($min == 0) {
        ...
        OpenTable();
        $testmessage = "";
        $result = $db->sql_query("SELECT `ip_lo`, `ip_hi` FROM `"._SENTINEL_EXCLUDED_RANGES_TABLE."` ORDER BY ip_lo LIMIT $min, $perpage");
        while(list($xip_lo, $xip_hi) = $db->sql_fetchrow($result)) {
            $test1 = $db->sql_query("SELECT * FROM "._SENTINEL_EXCLUDED_RANGES_TABLE." WHERE (ip_lo<='$xip_lo' AND ip_hi>='$xip_lo') AND (`ip_lo`!='$xip_lo' AND `ip_hi`!='$xip_hi') ORDER BY `ip_lo`");
            $test2 = $db->sql_query("SELECT * FROM "._SENTINEL_EXCLUDED_RANGES_TABLE." WHERE (ip_lo<='$xip_hi' AND ip_hi>='$xip_hi') AND (`ip_lo`!='$xip_lo' AND `ip_hi`!='$xip_hi') ORDER BY `ip_lo`");
            $test3 = $db->sql_query("SELECT * FROM "._SENTINEL_EXCLUDED_RANGES_TABLE." WHERE (ip_lo>='$xip_lo' AND ip_hi<='$xip_hi') AND (`ip_lo`!='$xip_lo' AND `ip_hi`!='$xip_hi') ORDER BY `ip_lo`");
            $test4 = $db->sql_query("SELECT * FROM "._SENTINEL_EXCLUDED_RANGES_TABLE." WHERE (ip_lo<='$xip_lo' AND ip_hi>='$xip_hi') AND (`ip_lo`!='$xip_lo' AND `ip_hi`!='$xip_hi') ORDER BY `ip_lo`");
        ...
        if($testmessage != "") {
            ...
            echo '{'."\n";
            echo ' location.href = \''.$admin_file.'.php?op=ABExcludedOverlapCheck&min='.$max.'\';'."\n";
            echo '}'."\n";
            echo '// --></script>'."\n";
