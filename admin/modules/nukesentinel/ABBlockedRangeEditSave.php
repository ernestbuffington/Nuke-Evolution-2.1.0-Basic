...
global $admin_file, $db, $_GETVAR, $ab_config, $bgcolor1, $bgcolor2;
...
if (is_admin()) {
    ...
    $old_ip_lo  = $_GETVAR->get('old_ip_lo', '_POST', 'int', 0);
    $old_ip_hi  = $_GETVAR->get('old_ip_hi', '_POST', 'int', 0);
    ...
    $test1 = $db->sql_query("SELECT * FROM "._SENTINEL_BLOCKED_RANGES_TABLE." WHERE (ip_lo<='$longip_lo' AND ip_hi>='$longip_lo') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
    $test2 = $db->sql_query("SELECT * FROM "._SENTINEL_BLOCKED_RANGES_TABLE." WHERE (ip_lo<='$longip_hi' AND ip_hi>='$longip_hi') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
    $test3 = $db->sql_query("SELECT * FROM "._SENTINEL_BLOCKED_RANGES_TABLE." WHERE (ip_lo>='$longip_lo' AND ip_hi<='$longip_hi') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
    $test4 = $db->sql_query("SELECT * FROM "._SENTINEL_BLOCKED_RANGES_TABLE." WHERE (ip_lo<='$longip_lo' AND ip_hi>='$longip_hi') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
    ...
    if($testnum1 > 0 OR $testnum2 >0 OR $testnum3 >0 OR $testnum4 >0) {
        ...
        $db->sql_uquery("UPDATE `"._SENTINEL_BLOCKED_RANGES_TABLE."` SET `ip_lo`='$longip_lo', `ip_hi`='$longip_hi', `c2c`='$xc2c', `date`='$xtime', `notes`='$xnotes', `expires`='$xexpires', `reason`='$xreason' WHERE `ip_lo`='$old_ip_lo' AND `ip_hi`='$old_ip_hi'");
