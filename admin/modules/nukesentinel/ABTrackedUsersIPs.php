...
if (is_admin()) {
    ...
    $totalselected = $db->sql_unumrows("SELECT DISTINCT(`ip_addr`) FROM `"._SENTINEL_TRACKED_IPS_TABLE."` WHERE `user_id`='$tid'");
    if ($totalselected > 0) {
        ...
        echo '</tr>'."\n";
        $result = $db->sql_query("SELECT DISTINCT(`ip_addr`) FROM `"._SENTINEL_TRACKED_IPS_TABLE."` WHERE `user_id`='$tid' ORDER BY `ip_long` LIMIT $min, $perpage");
        while(list($lipaddr) = $db->sql_fetchrow($result)){
            $newrow = $db->sql_ufetchrow("SELECT * FROM `"._SENTINEL_TRACKED_IPS_TABLE."` WHERE `user_id`='$tid' AND `ip_addr`='$lipaddr' ORDER BY `date` DESC LIMIT 1");
