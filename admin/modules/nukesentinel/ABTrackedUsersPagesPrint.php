...
if (is_admin()) {
    ...
    $totalselected = $db->sql_unumrows("SELECT `user_id`, `ip_addr`, `page`, `date` FROM `"._SENTINEL_TRACKED_IPS_TABLE."` WHERE `user_id`='$tid'");
    if($totalselected > 0) {
        ...
        echo '</tr>'."\n";
        $result = $db->sql_query("SELECT `user_id`, `ip_addr`, `page`, `date` FROM `"._SENTINEL_TRACKED_IPS_TABLE."` WHERE `user_id`='$tid' ORDER BY `date` DESC");
