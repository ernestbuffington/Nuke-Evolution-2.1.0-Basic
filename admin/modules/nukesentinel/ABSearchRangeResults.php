...
if (is_admin()) {
    ...
    $sip_lo     = $_GETVAR->get('sip_lo', '_REQUEST', 'array');
    $sip_hi     = $_GETVAR->get('sip_hi', '_REQUEST', 'array');
    ...
    if ($torun > 0) {
        ...
        $totalselected = $db->sql_unumrows("SELECT * FROM `"._SENTINEL_BLOCKED_IPS_TABLE."` WHERE `ip_long` >='$longip_lo' AND `ip_long`<='$longip_hi'");
        if ($totalselected > 0) {
            echo '<br />'."\n";
            OpenTable();
        ...
        $totalselected = $db->sql_unumrows("SELECT * FROM `"._SENTINEL_BLOCKED_RANGES_TABLE."` WHERE (`ip_lo`<='$longip_lo' AND `ip_hi`>='$longip_lo') OR (`ip_lo`<='$longip_hi' AND `ip_hi`>='$longip_hi') OR (`ip_lo`>='$longip_lo' AND `ip_hi`<='$longip_hi') OR (`ip_lo`<='$longip_lo' AND `ip_hi`>='$longip_hi') ORDER BY `ip_lo`");
        if ($totalselected > 0) {
            echo '<br />'."\n";
            OpenTable();
