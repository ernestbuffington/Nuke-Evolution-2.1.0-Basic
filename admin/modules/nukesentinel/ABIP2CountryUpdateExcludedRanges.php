...
if (is_admin()) {
    ...
    if ($min == 0) {
        ...
        $result = $db->sql_query("SELECT `ip_lo` FROM `"._SENTINEL_EXCLUDED_RANGES_TABLE."` LIMIT $min, $perpage");
        while(list($xip_lo) = $db->sql_fetchrow($result)) {
            ...
            $db->sql_uquery("UPDATE `"._SENTINEL_EXCLUDED_RANGES_TABLE."` SET `c2c`='$xc2c' WHERE `ip_lo`='$xip_lo'");
        }
