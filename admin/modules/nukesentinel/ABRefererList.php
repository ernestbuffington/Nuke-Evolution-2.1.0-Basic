...
if (is_admin()) {
    ...
    $totalselected = $db->sql_unumrows("SELECT * FROM `"._SENTINEL_REFERERS_TABLE."`");
    if ($totalselected > 0) {
        ...
        echo '</tr>'."\n";
        $result = $db->sql_query("SELECT * FROM `"._SENTINEL_REFERERS_TABLE."` ORDER BY `referer` $direction LIMIT $min,$perpage");
