...
global $admin_file, $db, $bgcolor2, $bgcolor1, $_GETVAR;
...
if (is_admin()) {
    ...
    $test1 = $db->sql_query("SELECT * FROM `"._SENTINEL_EXCLUDED_RANGES_TABLE."` WHERE `ip_lo`<='$longip_lo' AND `ip_hi`>='$longip_lo'");
    $test2 = $db->sql_query("SELECT * FROM `"._SENTINEL_EXCLUDED_RANGES_TABLE."` WHERE `ip_lo`<='$longip_hi' AND `ip_hi`>='$longip_hi'");
    $test3 = $db->sql_query("SELECT * FROM `"._SENTINEL_EXCLUDED_RANGES_TABLE."` WHERE `ip_lo`>='$longip_lo' AND `ip_hi`<='$longip_hi'");
    $test4 = $db->sql_query("SELECT * FROM `"._SENTINEL_EXCLUDED_RANGES_TABLE."` WHERE `ip_lo`<='$longip_lo' AND `ip_hi`>='$longip_hi'");
    ...
    if($testnum1 > 0 OR $testnum2 >0 OR $testnum3 >0 OR $testnum4 >0) {
        ...
        $xnotes = str_replace("<br />", "\r\n", $xnotes);
        $xtime = time();
        $db->sql_uquery("INSERT INTO `"._SENTINEL_EXCLUDED_RANGES_TABLE."` (`ip_lo`, `ip_hi`, `date`, `notes`, `c2c`) VALUES ('$longip_lo', '$longip_hi', '$xtime', '$xnotes', '$xc2c')");
        if($another == 1) {
            redirect($admin_file.'.php?op=ABExcludedAdd');
        }else {
            redirect($admin_file.'.php?op=ABExcludedList');
        }
    }
} else {
