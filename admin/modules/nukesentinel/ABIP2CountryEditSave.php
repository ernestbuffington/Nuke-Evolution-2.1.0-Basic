...
if (is_admin()) {
    ...
    $xip_lo     = $_GETVAR->get('xip_lo', '_REQUEST', 'array');
    $xip_hi     = $_GETVAR->get('xip_hi', '_REQUEST', 'array');
    $another    = $_GETVAR->get('another', '_REQUEST', 'int');
    $x2c2       = $_GETVAR->get('x2c2', '_REQUEST', 'string');
    ...
    list($xcountry) = $db->sql_ufetchrow("SELECT `country` FROM `"._SENTINEL_COUNTRIES_TABLE."` WHERE `c2c`='$xc2c' LIMIT 0,1");
    $test1 = $db->sql_query("SELECT * FROM "._SENTINEL_IP2COUNTRY_TABLE." WHERE (ip_lo<='$longip_lo' AND ip_hi>='$longip_lo') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
    $test2 = $db->sql_query("SELECT * FROM "._SENTINEL_IP2COUNTRY_TABLE." WHERE (ip_lo<='$longip_hi' AND ip_hi>='$longip_hi') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
    $test3 = $db->sql_query("SELECT * FROM "._SENTINEL_IP2COUNTRY_TABLE." WHERE (ip_lo>='$longip_lo' AND ip_hi<='$longip_hi') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
    $test4 = $db->sql_query("SELECT * FROM "._SENTINEL_IP2COUNTRY_TABLE." WHERE (ip_lo<='$longip_lo' AND ip_hi>='$longip_hi') AND (`ip_lo`!='$old_ip_lo' AND `ip_hi`!='$old_ip_hi') ORDER BY `ip_lo`");
    ...
    if($testnum1 > 0 OR $testnum2 >0 OR $testnum3 >0 OR $testnum4 >0) {
      ...
      $xtime = time();
      $db->sql_uquery("UPDATE `"._SENTINEL_IP2COUNTRY_TABLE."` SET `ip_lo`='$longip_lo', `ip_hi`='$longip_hi', `c2c`='$xc2c', `date`='$xtime' WHERE `ip_lo`='$old_ip_lo' AND `ip_hi`='$old_ip_hi'");
      $db->sql_uquery("UPDATE `"._SENTINEL_TRACKED_IPS_TABLE."` SET `c2c`='$xc2c' WHERE `ip_long` >= '$longip_lo' AND `ip_long` <= '$longip_hi'");
      $db->sql_uquery("UPDATE `"._SENTINEL_BLOCKED_IPS_TABLE."` SET `c2c`='$xc2c' WHERE `ip_long` >= '$longip_lo' AND `ip_long` <= '$longip_hi'");
      $db->sql_uquery("UPDATE `"._SENTINEL_BLOCKED_RANGES_TABLE."` SET `c2c`='$xc2c' WHERE `ip_lo` >= '$longip_lo' AND `ip_lo` <= '$longip_hi'");
