...
if (is_admin()) {
    ...
    $test1 = $db->sql_query("SELECT * FROM "._SENTINEL_IP2COUNTRY_TABLE." WHERE ip_lo<='$longip_lo' AND ip_hi>='$longip_lo' ORDER BY `ip_lo`");
    $test2 = $db->sql_query("SELECT * FROM "._SENTINEL_IP2COUNTRY_TABLE." WHERE ip_lo<='$longip_hi' AND ip_hi>='$longip_hi' ORDER BY `ip_lo`");
    $test3 = $db->sql_query("SELECT * FROM "._SENTINEL_IP2COUNTRY_TABLE." WHERE ip_lo>='$longip_lo' AND ip_hi<='$longip_hi' ORDER BY `ip_lo`");
    $test4 = $db->sql_query("SELECT * FROM "._SENTINEL_IP2COUNTRY_TABLE." WHERE ip_lo<='$longip_lo' AND ip_hi>='$longip_hi' ORDER BY `ip_lo`");
    ...
    if ($testnum1 > 0 OR $testnum2 >0 OR $testnum3 >0 OR $testnum4 >0) {
        ...
        $db->sql_uquery("INSERT INTO `"._SENTINEL_IP2COUNTRY_TABLE."` (`ip_lo`, `ip_hi`, `date`, `c2c`)  VALUES ('$longip_lo', '$longip_hi', '$datetime', '$xc2c')");
        list($xcountry) = $db->sql_ufetchrow("SELECT `country` FROM `"._SENTINEL_COUNTRIES_TABLE."` WHERE `c2c`='$xc2c'");
        ...
        if($another == 1) {
            redirect($admin_file.'.php?op=ABIP2CountryAdd');
        }else {
            redirect($admin_file.'.php?op=ABIP2CountryList');
        }
    }
} else {
