...
if (is_god_admin()) {
    ...
    echo '<tr><td bgcolor="'.$bgcolor2.'">'.evo_help_img(_AB_HELP_078).' '._AB_SHOWRIGHT.':</td>'."\n";
    echo '<td><select name="xshow_right">'."\n";
    $selshow1 = $selshow2 = '';
    if (isset($ip_sets['show_right']) && $ip_sets['show_right']==0) {
        $selshow1 = ' selected="selected"';
    } else {
        $selshow2 = ' selected="selected"';
    }
    echo '<option value="0"'.$selshow1.'>'._AB_NO.'</option>'."\n";
    echo '<option value="1"'.$selshow2.'>'._AB_YES.'</option>'."\n";
    echo '</select></td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'">'.evo_help_img(_AB_HELP_114).' '._AB_TESTSWITCH.':</td>'."\n";
    $seltest1 = $seltest2 = '';
    if ($ip_sets['test_switch'] == 1) {
        $seltest2 = ' selected="selected"';
    } else {
        $seltest1 = ' selected="selected"';
    }
    echo '<td><select name="xtest_switch">'."\n";
    echo '<option value="0"'.$seltest1.'>'._AB_NO.'</option>'."\n";
    echo '<option value="1"'.$seltest2.'>'._AB_YES.'</option>'."\n";
    echo '</select></td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'">'.evo_help_img(_AB_HELP_048).' '._AB_DISABLESWITCH.':</td>'."\n";
    $seldable1 = $seldable2 = '';
    if ($ip_sets['disable_switch'] == 1) {
        $seldable2 = ' selected="selected"';
    } else {
        $seldable1 = ' selected="selected"';
    }
    ...
    echo '</select></td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'" valign="top">'.evo_help_img(_AB_HELP_003).' '._AB_SITESWITCH.':</td>'."\n";
    echo '<td><select name="xsite_switch">'."\n";
    $sel1 = $sel2 = '';
    if ($ip_sets['site_switch']==1) {
    ...
    echo '</select></td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'">'.evo_help_img(_AB_HELP_004).' '._AB_TEMPLATE.':</td>'."\n";
    ...
    for ($i=0; $i < sizeof($templatelist); $i++) {
        if($templatelist[$i]!="") {
            ...
            echo '<option';
            if ($templatelist[$i]==$ip_sets['site_reason']) {
    ...
    echo '</select></td></tr>'."\n";
    $seldns1 = $seldns2 = $seldns3 = "";
    echo '<tr><td bgcolor="'.$bgcolor2.'" valign="top">'.evo_help_img(_AB_HELP_086).' '._AB_IPLOOKUPSITE.':</td>'."\n";
    if (stristr($ip_sets['lookup_link'], "arin.net")) {
        $seldns1 = ' selected="selected"';
    } elseif (stristr($ip_sets['lookup_link'], "network-tools")) {
        $seldns2 = ' selected="selected"';
    } else {
        $seldns3 = ' selected="selected"';
    }
    echo '<td><select name="xlookup_link">'."\n";
    echo '<option value="http://ws.arin.net/cgi-bin/whois.pl?queryinput="'.$seldns1.'>Arin Net</option>'."\n";
    echo '<option value="http://network-tools.com/default.asp?prog=whois&amp;host="'.$seldns2.'>Network Tools</option>'."\n";
    echo '<option value="http://www.db.ripe.net/fcgi-bin/whois?form_type=simple&amp;full_query_string=&amp;do_search=Search&amp;searchtext="'.$seldns3.'>Ripe Net</option>'."\n";
    echo '</select></td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'">'.evo_help_img(_AB_HELP_047).' '._AB_FORCENUKEURL.':</td>'."\n";
    echo '<td><select name="xforce_nukeurl">'."\n";
    $selforce1 = $selforce2 = '';
    if ($ip_sets['force_nukeurl']==0) {
        $selforce1 = ' selected="selected"';
    } else {
        $selforce2 = ' selected="selected"';
    }
    echo '<option value="0"'.$selforce1.'>'._AB_NO.'</option>'."\n";
    echo '<option value="1"'.$selforce2.'>'._AB_YES.'</option>'."\n";
    echo '</select></td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'">'.evo_help_img(_AB_HELP_118).' '._AB_PAGEDELAY.':</td>'."\n";
    echo '<td><select name="xpage_delay">'."\n";
    $i=1;
    while($i<=10) {
        echo '<option value="'.$i.'"';
        if ($ip_sets['page_delay']==$i) {
            echo ' selected="selected"';
        }
        echo '>'.$i.'</option>'."\n";
        $i++;
    ...
    echo '<tr><td bgcolor="'.$bgcolor2.'" valign="top">'.evo_help_img(_AB_HELP_072).' '._AB_FLOODDELAY.':</td>'."\n";
    echo '<td><select name="xflood_delay">'."\n";
    $i=1;
    while ($i<=5) {
        echo '<option value="'.$i.'"';
        if ($ip_sets['flood_delay']==$i) {
            echo ' selected="selected"';
        }
        echo '>'.$i.'</option>'."\n";
        $i++;
    ...
    echo '<tr><td bgcolor="'.$bgcolor2.'">'.evo_help_img(_AB_HELP_001).' '._AB_DISPLAYLINK.':</td>'."\n";
    echo '<td><select name="xdisplay_link">'."\n";
    $sel1 = $sel2 = $sel3 = $sel4 = '';
    if ($ip_sets['display_link']==1) {
        $sel2 = ' selected="selected"';
    } elseif ($ip_sets['display_link']==2) {
        $sel3 = ' selected="selected"';
    } elseif ($ip_sets['display_link']==3) {
    ...
    echo '</select></td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'">'.evo_help_img(_AB_HELP_002).' '._AB_DISPLAYREASON.':</td>'."\n";
    echo '<td><select name="xdisplay_reason">'."\n";
    $sel1 = $sel2 = $sel3 = $sel4 = '';
    if ($ip_sets['display_reason']==1) {
        $sel2 = ' selected="selected"';
    } elseif ($ip_sets['display_reason']==2) {
        $sel3 = ' selected="selected"';
    } elseif ($ip_sets['display_reason']==3) {
        $sel4 = ' selected="selected"';
    } else {
        $sel1 = ' selected="selected"';
    }
    ...
    echo '<option value="3"'.$sel4.'>'._AB_VISITORS.'</option>'."\n";
    echo '</select></td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'" valign="top">'.evo_help_img(_AB_HELP_044).' '._AB_PROXYBLOCKER.':</td>'."\n";
    $selproxy1 = $selproxy2 = $selproxy3 = $selproxy4 = '';
    if ($ip_sets['proxy_switch'] == 1) {
        $selproxy2 = ' selected="selected"';
    } elseif ($ip_sets['proxy_switch'] == 2) {
        $selproxy3 = ' selected="selected"';
    } elseif ($ip_sets['proxy_switch'] == 3) {
        $selproxy4 = ' selected="selected"';
    ...
    echo '</select></td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'">'.evo_help_img(_AB_HELP_045).' '._AB_TEMPLATE.':</td>'."\n";
    ...
    for($i=0; $i < sizeof($templatelist); $i++) {
        if($templatelist[$i]!="") {
            ...
            $bl = preg_replace('#_#', ' ', $bl);
            echo '<option';
            if ($templatelist[$i]==$ip_sets['proxy_reason']) {
                echo ' selected="selected"';
            }
            echo ' value="'.$templatelist[$i].'">'.ucfirst($bl).'</option>'."\n";
        }
    ...
    echo '</select></td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'" valign="top">'.evo_help_img(_AB_HELP_042).' '._AB_SELFEXPIRE.':</td>'."\n";
    $selexpire1 = $selexpire2 = '';
    if ($ip_sets['self_expire'] == 1) {
        $selexpire2 = ' selected="selected"';
    } else {
        $selexpire1 = ' selected="selected"';
    }
    ...
    echo '</select></td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'" valign="top">'.evo_help_img(_AB_HELP_056).' '._AB_SANTYPROTECTION.':</td>'."\n";
    $selsanty1 = $selsanty2 = '';
    if ($ip_sets['santy_protection'] == 1) {
        $selsanty2 = ' selected="selected"';
    } else {
        $selsanty1 = ' selected="selected"';
    }
    ...
    echo '</select></td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'" valign="top">'.evo_help_img(_AB_HELP_046).' '._AB_PREVENTDOS.':</td>'."\n";
    $seldos1 = $seldos2 = '';
    if ($ip_sets['prevent_dos'] == 1) {
        $seldos2 = ' selected="selected"';
    } else {
        $seldos1 = ' selected="selected"';
    }
    ...
    echo '<tr><td bgcolor="'.$bgcolor2.'" valign="top">'.evo_help_img(_AB_HELP_007).' '._AB_ADMINAUTH.':</td>'."\n";
    $apass = $db->sql_unumrows("SELECT * FROM `"._SENTINEL_ADMINS_TABLE."` WHERE `password_md5`='' OR `password`='' OR `password_crypt`=''");
    $sapi_name = strtolower(php_sapi_name());
    $selauth1 = $selauth2 = $selauth3 = '';
    if ($ip_sets['http_auth'] == 1) {
        $selauth2 = ' selected="selected"';
    } elseif ($ip_sets['http_auth'] == 2) {
        $selauth3 = ' selected="selected"';
    ...
    echo '</select>';
    if ($apass > 0) {
        echo '<br /><strong>'._AB_SETPASSWORDS.'</strong>';
    }
    echo '</td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'" valign="top">'.evo_help_img(_AB_HELP_005).' '._AB_HTACCESSPATH.':</td>'."\n";
    echo '<td>';
    if(stristr($_SERVER['SERVER_SOFTWARE'], "apache")) {
        ...
        if ($ip_sets['htaccess_path']>"") {
            ...
            if (!$httest) {
                echo '<br /><strong>'._AB_HTWARNING.'</strong>';
            }
        }
    } else {
    ...
    echo '</td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'" valign="top">'.evo_help_img(_AB_HELP_057).' '._AB_STACCESSPATH.':<br /><a href="'.$admin_file.'.php?op=ABCGIAuth" target="_blank">'._AB_CGIAUTHSETUP.'</a></td>'."\n";
    echo '<td>';
    if (stristr($_SERVER['SERVER_SOFTWARE'], "apache")) {
        ...
        if ($ip_sets['staccess_path']>"") {
            ...
            if (!$sttest) {
                echo '<br /><strong>'._AB_STWARNING.'</strong>';
            }
        }
    } else {
    ...
    echo '</td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'" valign="top">'.evo_help_img(_AB_HELP_119).' '._AB_FTACCESSPATH.':</td>'."\n";
    echo '<td>';
    if (stristr($_SERVER['SERVER_SOFTWARE'], "apache")) {
    ...
    echo '</td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'" valign="top">'.evo_help_img(_AB_HELP_121).' '._AB_DUMPDIRECTORY.':<br /></td>'."\n";
    echo '<td>';
    ...
    if($ip_sets['dump_directory'] > '') {
        ...
        if(!$cachetest) {
            echo '<br /><strong>'._AB_DUMPWARNING.'</strong>';
        }
    }
    echo '</td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'">'.evo_help_img(_AB_HELP_067).' '._AB_CRYPTSALT.':</td>'."\n";
    echo '<td><input type="text" name="xcrypt_salt" size="3" maxlength="2" value="'.$ip_sets['crypt_salt'].'" /></td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'" valign="top">'.evo_help_img(_AB_HELP_006).' '._AB_ADMINLIST.':</td>'."\n";
    ...
    echo '<tr><td bgcolor="'.$bgcolor2.'">'.evo_help_img(_AB_HELP_050).' '._AB_IPTRACKER.':</td>'."\n";
    echo '<td><select name="xtrack_active">'."\n";
    $sel1 = $sel2 = '';
    if ($ip_sets['track_active']==0) {
        $sel1 = ' selected="selected"';
    } else {
        $sel2 = ' selected="selected"';
    }
    ...
    echo '</select></td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'">'.evo_help_img(_AB_HELP_051).' '._AB_MAXIMUMDAYS.':</td>'."\n";
    echo '<td><select name="xtrack_max">'."\n";
    $selmax='';
    if ($ip_sets['track_max']==0) {
        $selmax = _AB_UNLIMITED;
    }
    ...
    echo '<tr><td bgcolor="'.$bgcolor2.'">'.evo_help_img(_AB_HELP_008).' '._AB_IPSPERPAGE.':</td>'."\n";
    echo '<td><input type="text" name="xblock_perpage" size="5" value="'.$ip_sets['block_perpage'].'" /></td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'">'.evo_help_img(_AB_HELP_009).' '._AB_SORTCOLUMN.':</td>'."\n";
    echo '<td><select name="xblock_sort_column">'."\n";
    $selcolumn1 = $selcolumn2 = $selcolumn3 = $selcolumn4 = $selcolumn5 = '';
    if ($ip_sets['block_sort_column'] == "expires") {
        $selcolumn2 = ' selected="selected"';
    } elseif ($ip_sets['block_sort_column'] == "date") {
        $selcolumn3 = ' selected="selected"';
    } elseif ($ip_sets['block_sort_column'] == "reason") {
        $selcolumn4 = ' selected="selected"';
    } elseif ($ip_sets['block_sort_column'] == "c2c") {
    ...
    echo '</select></td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'">'.evo_help_img(_AB_HELP_010).' '._AB_SORTDIRECTION.':</td>'."\n";
    echo '<td><select name="xblock_sort_direction">'."\n";
    $seldirection1 = $seldirection2 = '';
    if ($ip_sets['block_sort_direction'] == "desc") {
    ...
    echo '<tr><td bgcolor="'.$bgcolor2.'">'.evo_help_img(_AB_HELP_053).' '._AB_IPSPERPAGE.':</td>'."\n";
    echo '<td><input type="text" name="xtrack_perpage" size="5" value="'.$ip_sets['track_perpage'].'" /></td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'">'.evo_help_img(_AB_HELP_054).' '._AB_SORTCOLUMN.':</td>'."\n";
    echo '<td><select name="xtrack_sort_column">'."\n";
    $selcolumn1 = $selcolumn2 = $selcolumn3 = $selcolumn4 = $selcolumn5 = $selcolumn6 = '';
    if ($ip_sets['track_sort_column'] == "date") {
        $selcolumn3 = ' selected="selected"';
    } elseif ($ip_sets['track_sort_column'] == "username") {
        $selcolumn4 = ' selected="selected"';
    } elseif ($ip_sets['track_sort_column'] == 5) {
        $selcolumn5 = ' selected="selected"';
    } elseif ($ip_sets['track_sort_column'] == "c2c") {
    ...
    echo '</select></td></tr>'."\n";
    echo '<tr><td bgcolor="'.$bgcolor2.'">'.evo_help_img(_AB_HELP_055).' '._AB_SORTDIRECTION.':</td>'."\n";
    echo '<td><select name="xtrack_sort_direction">'."\n";
    $seldirection1 = $seldirection2 = '';
    if ($ip_sets['track_sort_direction'] == "desc") {
        $seldirection2 = ' selected="selected"';
    } else {
        $seldirection1 = ' selected="selected"';
    }
