...
if (is_admin()) {
    ...
    if($totalselected > 0) {
        ...
        echo '</tr>'."\n";
        $result = $db->sql_query("SELECT DISTINCT(`user_agent`), tid, MAX(`date`), COUNT(*) FROM `"._SENTINEL_TRACKED_IPS_TABLE."` GROUP BY 1 ORDER BY $column $direction LIMIT $min, $perpage");
        while(list($user_agent, $tid, $lastview, $hits) = $db->sql_fetchrow($result)){
            ...
            echo '<a href="'.$admin_file.'.php?op=ABTrackedAgentsDelete&amp;tid='.$tid.'&amp;min='.$min.'&amp;column='.$column.'&amp;direction='.$direction.'"><img src="images/nukesentinel/delete.png" height="16" width="16" alt="'._AB_DELETE.'" title="'._AB_DELETE.'" border="0" /></a></td>'."\n";
            echo '</tr>'."\n";
        }
