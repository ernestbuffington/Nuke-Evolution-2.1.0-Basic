...
if (is_admin()) {
    ...
    $totalselected = $db->sql_unumrows("SELECT `date` FROM `"._SENTINEL_EXCLUDED_RANGES_TABLE."`");
    if($totalselected > 0) {
        ...
        echo '</tr>'."\n";
        $result = $db->sql_query("SELECT * FROM `"._SENTINEL_EXCLUDED_RANGES_TABLE."` ORDER BY $column $direction LIMIT $min,$perpage");
