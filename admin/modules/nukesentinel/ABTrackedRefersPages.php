...
if (is_admin()) {
    ...
    $ip_addr    = $_GETVAR->get('ip_addr', '_REQUEST', 'int');
    ...
    $totalselected = $db->sql_unumrows("SELECT * FROM `"._SENTINEL_TRACKED_IPS_TABLE."` WHERE `refered_from`='$uname'");
    if($totalselected > 0) {
        if (strlen($uname) > 50) {
            $rfrom = substr($uname, 0, 50)."...";
        } else {
            $rfrom = UsernameColor($uname);
        }
        ...
        echo '</tr>'."\n";
        $result = $db->sql_query("SELECT `ip_addr`, `page`, `date` FROM `"._SENTINEL_TRACKED_IPS_TABLE."` WHERE `refered_from`='$uname' ORDER BY $column $direction LIMIT $min, $perpage");
