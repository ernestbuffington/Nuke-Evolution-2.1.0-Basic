...
global $admin_file, $db, $ab_config, $bgcolor2, $bgcolor1, $_GETVAR;
...
if (is_admin()) {
    ...
    $totalselected = $db->sql_unumrows("SELECT DISTINCT(`ip_addr`) FROM `"._SENTINEL_TRACKED_IPS_TABLE."` WHERE `user_agent`='$uname'");
    if($totalselected > 0) {
            ...
            echo '</tr>'."\n";
            $result = $db->sql_query("SELECT DISTINCT(`ip_addr`) FROM `"._SENTINEL_TRACKED_IPS_TABLE."` WHERE `user_agent`='$uname' ORDER BY `ip_long` LIMIT $min, $perpage");
            while(list($lipaddr) = $db->sql_fetchrow($result)){
                $newrow = $db->sql_ufetchrow("SELECT * FROM `"._SENTINEL_TRACKED_IPS_TABLE."` WHERE `user_agent`='$uname' AND `ip_addr`='$lipaddr' ORDER BY `date` DESC LIMIT 1");
