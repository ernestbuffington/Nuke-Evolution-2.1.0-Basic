...
if (is_admin()) {
    ...
    if ($totalselected > 0) {
        ...
        echo '</tr>'."\n";
        $result = $db->sql_query("SELECT DISTINCT(`refered_from`), tid, MAX(`date`), COUNT(*) FROM `"._SENTINEL_TRACKED_IPS_TABLE."` GROUP BY 1 ORDER BY `refered_from`");
        while(list($refered_from, $tid, $lastview, $hits) = $db->sql_fetchrow($result)){
            ...
            $refered_from = str_replace("&amp;amp;", "&amp;", $refered_from);
