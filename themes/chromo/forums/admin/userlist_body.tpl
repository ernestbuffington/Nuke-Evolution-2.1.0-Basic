        ...
        <tr>
            ...
            <td nowrap="nowrap">
                <select name="sort" class="post">
                    <option value="user_id" {SELECTED_USER_ID}>{L_USER_ID}</option>
                    <option value="user_active" {SELECTED_ACTIVE}>{L_ACTIVE}</option>
                    <option value="username" {SELECTED_USERNAME}>{L_USERNAME}</option>
                    <option value="user_regdate" {SELECTED_JOINED}>{L_JOINED}</option>
                    <option value="user_lastvisit" {SELECTED_ACTIVTY}>{L_ACTIVTY}</option>
                    <option value="user_level" {SELECTED_USER_LEVEL}>{L_USER_LEVEL}</option>
                    <option value="user_posts" {SELECTED_POSTS}>{L_POSTS}</option>
                    <option value="user_rank" {SELECTED_RANK}>{L_RANK}</option>
                    <option value="user_email" {SELECTED_EMAIL}>{L_EMAIL}</option>
                    <option value="user_website" {SELECTED_WEBSITE}>{L_WEBSITE}</option>
                </select>
            </td>
            <td nowrap="nowrap">
...
<!-- BEGIN user_row -->
        <tr>
            <td class="{user_row.ROW_CLASS}" nowrap="nowrap">
                <input type="checkbox" name="{S_USER_VARIABLE}[]" value="{user_row.USER_ID}" />&nbsp;&nbsp;&nbsp;
                <a href="javascript:handleClick('user{user_row.USER_ID}');">{L_OPEN_CLOSE}</a>
            </td>
            <td class="{user_row.ROW_CLASS}">
                <span class="gen"
                    {user_row.STYLE_COLOR}>
                    <strong><a href="{user_row.U_PROFILE}" class="gen" target="_blank" {user_row.STYLE_COLOR}>{user_row.USERNAME}</a></strong>
                </span>
            </td>
            <td class="{user_row.ROW_CLASS}">
                <span class="gen">
            ...
            <td class="{user_row.ROW_CLASS}">
                <span class="gen">
                    {user_row.LAST_ACTIVITY}
                </span>
            </td>
        </tr>
        <tr id="user{user_row.USER_ID}" style="display: none">
            <td class="{user_row.ROW_CLASS}" width="13%">&nbsp;
                
            </td>
            <td class="{user_row.ROW_CLASS}" colspan="4" width="100%">
                <table  width="100%" cellpadding="3" cellspacing="1" >
                    <tr>
                        <td class="{user_row.ROW_CLASS}" width="33%">
                            <span class="gen">
                                <strong>{L_RANK}:</strong>&nbsp;{user_row.RANK}&nbsp;{user_row.I_RANK}
                            </span>
                        </td>
                        <td class="{user_row.ROW_CLASS}" width="34%">
                            <span class="gen">
                                <strong>{L_GROUP}:</strong>
<!-- BEGIN group_row -->
...
<!-- END no_group_row -->
                        ...
                        <td class="{user_row.ROW_CLASS}" width="33%">
                            <span class="gen">
                                <strong>{L_POSTS}:</strong>&nbsp;{user_row.POSTS}&nbsp;<a href="{user_row.U_SEARCH}" class="gen" target="_blank">{L_FIND_ALL_POSTS}</a>
                            </span>
                        </td>
                    </tr>
                    <tr>
                        <td class="{user_row.ROW_CLASS}" colspan="3">
                            <span class="gen">
                                <strong>{L_WEBSITE}:</strong>&nbsp;<a href="{user_row.U_WEBSITE}" class="gen" target="_blank">{user_row.U_WEBSITE}</a>
                            </span>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td colspan="2" class="{user_row.ROW_CLASS}">
                            {user_row.I_AVATAR}
                        </td>
                    </tr>
                </table>
