...
function OpenTable2() {
    ...
    echo "<table border='0' cellspacing='1' cellpadding='0' align='center'>\n";
    echo "  <tr>\n";
    echo "      <td class='extras'>\n";
...
function themecenterbox($title, $content) {
    OpenTable();
    echo '<div style="text-align:center;"><span class="option" style="font-weight:bold;">'.$title.'</span></div><br />'.$content;
    CloseTable();
    echo '<br />';
}

function themepreview($title, $hometext, $bodytext='', $notes='') {
    echo '<span style="font-weight:bold;">'.$title.'</span><br /><br />'.$hometext;
    if (!empty($bodytext)) {
        echo '<br /><br />'.$bodytext;
    }
    if (!empty($notes)) {
        echo '<br /><br /><span style="font-weight:bold;">'._NOTE.'</span>&nbsp;<em>'.$notes.'</em>';
    }
}
