...
global $userinfo, $db, $ThemeSel, $board_config, $currentlang, $images, $lang_block;
...
function make_EvoTopicsrow($rowimage, $row_class, $topic_id, $post_time, $topic_title, $topic_poster, $topic_views, $topic_replies, $topic_icon, $topic_postername, $forum_name, $forum_id, $topic_last_post_id, $post_username, $poster_id) {
    ...
    $row = "<tr>"
             ...
             ."<td align=\"left\" class=\"$row_class\"><a href=\"modules.php?name=Forums&amp;file=viewtopic&amp;t=$topic_id#$topic_last_post_id\">$topic_title</a></td>"
            ."<td align=\"center\" class=\"$row_class\"><a href=\"modules.php?name=Forums&amp;file=profile&amp;mode=viewprofile&amp;u=$topic_poster\">$topic_postername</a></td>"
...
function make_EvoTopicstable($topicrows, $announcements, $SplitAnnouncements, $count_topics, $count_posts, $count_forum) {
    global $lang_block;

    $table = "<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"1\">";
        $table .= "<tr><td class=\"row3\" colspan=\"7\" height=\"15\" align=\"center\">" . $lang_block['BLOCK_FORUM_WEHAVE'] . "  $count_forum " . $lang_block['BLOCK_FORUM_FORUMS'] . ", $count_topics " . $lang_block['BLOCK_FORUM_TOPICS'] . ",  $count_posts " . $lang_block['BLOCK_FORUM_POSTS'] . "</td></tr>"
                   ...
                   ."<tr>"
                     ."<td><table width=\"100%\" border=\"0\" cellpadding=\"1\" cellspacing=\"1\" class=\"forumline\">"
                       ."<tr>"
                         ...
                         ."<th width=\"20%\" align=\"center\" nowrap=\"nowrap\" class=\"thtop\"><span class=\"block-title\"><strong>" . $lang_block['BLOCK_FORUM_CREATEDBY'] . "</strong></span></th>"
    ...
    if ($SplitAnnouncements) {
        $table .= "<tr><th class=\"thTop\" colspan=\"6\" height=\"28\" align=\"left\"><span class=\"cattitle\">" . $lang_block['BLOCK_FORUM_ANNOUNCE'] . "</span></th></tr>";
        $table .= ($announcements) ? $announcements : "<tr><td class=\"row3\" colspan=\"6\" height=\"15\">" . $lang_block['BLOCK_FORUM_ANNOUNCENO'] . "</td></tr>";
    }
    $table .= ($SplitAnnouncements) ? "<tr><th class=\"thTop\" colspan=\"6\" height=\"28\" align=\"left\"><span class=\"cattitle\">" . $lang_block['BLOCK_FORUM_TOPICS'] . "</span></th></tr>" : "";
    $table .= ($topicrows) ? $topicrows : "<tr><td class=\"row3\" colspan=\"6\" height=\"15\">" . $lang_block['BLOCK_FORUM_NOTOPIC'] . "</td></tr>";
    $table .= "<tr>"
             ."<th height=\"28\" colspan=\"6\" align=\"right\" class=\"catbottom\">[ <a href=\"modules.php?name=Forums&amp;file=recent\">" . $lang_block['BLOCK_FORUM_RECENT'] . "</a> ]&nbsp;</th>"
           ."</tr>"
         ."</table></td>"
         ."</tr>"
       ."</table></td>"
...
$sql = "SELECT
       f.forum_id, f.forum_name, t.topic_id, t.topic_title, t.topic_last_post_id, t.topic_poster, t.topic_views, t.topic_replies, t.topic_type, t.topic_status, t.topic_icon,
   ...
   FROM "
       ...
       . _USERS_TABLE . " u2
   WHERE
       f.forum_id NOT IN (" . $forumsignore . ")
...
while ( list( $forum_id, $forum_name, $topic_id, $topic_title, $topic_last_post_id, $topic_poster, $topic_views, $topic_replies, $topic_type, $topic_status, $topic_icon,
    ...
    if ( $topic_type == POST_GLOBAL_ANNOUNCE || $topic_type == POST_ANNOUNCE ) {
        if ( $Count_Announce <= $NumAnnouncements ) {
            ...
            $announcements .= make_EvoTopicsrow($rowimage, $row_class, $topic_id, $post_time, $topic_title, $topic_poster, $topic_views, $topic_replies, $topic_icon, $topic_postername, $forum_name, $forum_id, $topic_last_post_id, $post_username, $poster_id);
            $Count_Announce += 1;
        }
    } else {
        if ($topic_status == TOPIC_LOCKED ) {
             $rowimage = ($post_time >= $userinfo['user_lastvisit']) ? str_replace("{LANG}", "lang_".$currentlang, $images['folder_locked_new']) : str_replace("{LANG}", "lang_".$currentlang, $images['folder_locked']);
        } else {
            $rowimage = ($post_time >= $userinfo['user_lastvisit']) ? str_replace("{LANG}", "lang_".$currentlang, $images['folder_new']) : str_replace("{LANG}", "lang_".$currentlang, $images['folder']);
        }
        $post_time = formatTimestamp($post_time);
        $topicrows .= make_EvoTopicsrow($rowimage, $row_class, $topic_id, $post_time, $topic_title, $topic_poster, $topic_views, $topic_replies, $topic_icon, $topic_postername, $forum_name, $forum_id, $topic_last_post_id, $post_username, $poster_id);
    }
}
...
$content = make_EvoTopicstable($showrows, $announcements, $SplitAnnouncements, $count_topics, $count_posts, $count_forum);
