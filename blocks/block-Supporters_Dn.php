...
function block_SupportersDn_configs(){
  global $db, $cache;
...
if ( empty($supportersconfig) ) {
    $supportersconfig = block_SupportersDn_configs();
}

function block_SupportersDn_cache($block_cachetime, $supportersconfig) {
    global $db, $cache;
    if ((($blockcache = $cache->load('supporters', 'blocks')) === false) || empty($blockcache) || intval($blockcache[0]['stat_created']) < (time() - intval($block_cachetime))) {
        ...
        while(list($site_id, $site_name, $site_url, $site_image, $image_type, $site_date, $site_description, $site_hits) = $db->sql_fetchrow($result)) {
            ...
            if($height > $supportersconfig['max_height'] || ($height <= 0) ) {
            ...
            $blockcache[$a]['imgattr'] = array('site_id' => $site_id, 'site_name' => $site_name, 'site_url' => $site_url, 'site_image' => $site_image, 'site_date' => $site_date, 'site_description' => $site_description,
                                                'site_hits' => $site_hits, 'width' => $width, 'height' => $height, 'attr' => $attr, 'img_type' => $img_type);
        }
...
$blocksession = block_SupportersDn_cache($evoconfig['block_cachetime'], $supportersconfig);
...
if (empty($blockcontent)) {
    ...
    $content .= evo_marquee('block_SupportersDn', '100%', '100%', $blockcontent, 'down', 2, '100%', '100%' , 0, 0);
