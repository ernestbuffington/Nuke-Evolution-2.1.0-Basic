...
function block_SupportersRt_configs(){
  global $db, $cache;
...
if ( empty($supportersconfig) ) {
    $supportersconfig = block_SupportersRt_configs();
}

function block_SupportersRt_cache($block_cachetime, $supportersconfig) {
    global $db, $cache;
    if ((($blockcache = $cache->load('supporters', 'blocks')) === false) || empty($blockcache) || intval($blockcache[0]['stat_created']) < (time() - intval($block_cachetime))) {
        $image_atts = array();
        $result = $db->sql_query("SELECT `site_id`, `site_name`, `site_url`, `site_image`, `image_type`, `site_date`, `site_description`, `site_hits` FROM `"._NSNSP_SITES_TABLE."` WHERE `site_status`>'0' ORDER BY `site_id` DESC");
        $a = 0;
        ...
        $cache->save('supporters', 'blocks', $blockcache);
    }
    return $blockcache;
}
...
$blocksession = block_SupportersRt_cache($evoconfig['block_cachetime'], $supportersconfig);

$blockcontent = "<span style='width: 100%; height: ".($supportersconfig['max_height'] + 25)."px;'>\n";
$blockfoot    = '';
for ($a = 1, $max = count($blocksession); $a < $max; $a++) {
    ...
    $blockcontent .= "<a href='modules.php?name=$module_name&amp;op=SPGo&amp;site_id=".$site_id."'>\n";
    $blockcontent .= "<img src='".((intval($image_type) == 1) ? NUKE_MODULES_IMAGE_DIR.$site_image : $site_image)."' border='0' alt='' title='$site_name' height='".$height."px' width='".$width."px' /></a>&nbsp;&nbsp;\n";
}
$blockcontent .= "</span>\n";
...
if (empty($blockcontent)) {
    ...
    $content .= "<p style='text-align: center;'>".$lang_new[$module_name]['SP_SUPPORTEDBY']."</p>";
    $content .= "<div style='width: 100%; height: ".($supportersconfig['max_height'] + 25)."px;'>\n";
    $content .= evo_marquee('block_SupportersRt', '100%', '100%', $blockcontent, 'right', 2, '100%', '100%' , 0, 0);
