...
function Block_Top10WebLinksConfig() {
...
if ( empty($webblocklinksconfig) ) {
    $webblocklinksconfig = Block_Top10WebLinksConfig();
}

function block_top10links_cache($block_cachetime, $webblocklinksconfig) {
    global $db, $cache;
    if ((($blockcache = $cache->load('top10link', 'blocks')) === false) || empty($blockcache) || intval($blockcache[0]['stat_created']) < (time() - intval($block_cachetime))) {
        $result = $db->sql_query("SELECT `lid`, `title`, `image`, `url` FROM `"._WEBLINKS_LINKS_TABLE."` ORDER BY `hits` DESC LIMIT 0,".$webblocklinksconfig['block_rows']);
        $a = 0;
        while (list($lid, $title,$image, $url) = $db->sql_fetchrow($result)) {
            $a++;
            $blockcache[$a]['lid']   = $lid;
            $blockcache[$a]['title'] = $title;
            $blockcache[$a]['image'] = htmlentities($image);
            $blockcache[$a]['url']   = $url;
        }
        $db->sql_freeresult($result);
        $blockcache[0]['stat_created'] = time();
        $cache->save('top10link', 'blocks', $blockcache);
    }
    return $blockcache;
}

$blocksession = block_top10links_cache($evoconfig['block_cachetime'], $webblocklinksconfig);
...
for ($a = 1, $max = count($blocksession); $a < $max; $a++) {
    $title  = check_html($blocksession[$a]['title'], "nohtml");
    $url    = $blocksession[$a]['url'];
    $lid    = intval($blocksession[$a]['lid']);
    $image  = $blocksession[$a]['image'];
    $blockcontent .= "<div style='text-align:center; font-size: x-small;'>".$title."</div>";
    $blockcontent .= "<div style='margin-left: auto; margin-right: auto; text-align:center; width: 100%;'>";
    $blockcontent .= "<a href=\"modules.php?name=".$module_name."&amp;op=visit&amp;lid=$lid\" target=\"_blank\"></a>";
    if($image != 'http://' && !empty($image) ){
        $blockcontent .= "<a href=\"modules.php?name=$module_name&amp;op=linkvisit&amp;lid=$lid\" target=\"_blank\" ><img src=\"$image\"  width=\"".$webblocklinksconfig['block_image_width']."\" height=\"".$webblocklinksconfig['block_image_height']."\" border=\"0\" title=\"".$lang_block['BLOCK_WEBLINKS_FULL_VIEW']."\" alt=\"\" /></a>";
...
if (empty($blockcontent)) {
    ...
    if ( $webblocklinksconfig['block_scroll'] == '1') {
        ...
        $content .= evo_marquee('block_Top10Links', $webblocklinksconfig['block_height'], '100%', $blockcontent, $direction, $webblocklinksconfig['block_scroll_amount'], $webblocklinksconfig['block_height'], '100%' , 1, 0);
        $content .= "</div>\n";
    } else {
