...
function block_Supporters_configs(){
  global $db, $cache;
...
if ( empty($supportersconfigg) ) {
    $supportersconfig = block_Supporters_configs();
}

function block_Supporters_cache($block_cachetime, $supportersconfig) {
    global $db, $cache;
...
$blocksession = block_Supporters_cache($evoconfig['block_cachetime'], $supportersconfig);
...
if (is_active($module_name)) {
    $blockfoot .= "[ <a href='modules.php?name=Supporters'>".$lang_new[$module_name]['SP_SUPPORTERS']."</a> ]</p>\n";
}
...
if (empty($blockcontent)) {
    ...
    $content .= evo_marquee('block_Supporters', '100%', '100%', $blockcontent, 'down', 1, '100%', '100%' , 0, 0);
