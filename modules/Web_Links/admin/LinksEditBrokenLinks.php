...
echo "<form method=\"post\" action=\"".$admin_file.".php\" name=\"linkseditbroken\">";
echo "<table width=\"100%\" border=\"0\">\n";
$row = $db->sql_fetchrow($db->sql_query("SELECT `lid`, `cid`, `title`, `image`, `url`, `description`, `modifysubmitter` FROM `"._WEBLINKS_MODREQUEST_TABLE."` WHERE `brokenlink`='1' ORDER BY `requestid`"));
$lid = intval($row['lid']);
$cid = intval($row['cid']);
...
$row2 = $db->sql_fetchrow($db->sql_query("SELECT `name`, `email`, `hits` FROM `"._WEBLINKS_LINKS_TABLE."` WHERE `lid`='$lid'"));
$name = $row2['name'];
$email = $row2['email'];
$hits = intval($row2['hits']);
echo "<tr><td bgcolor='$bgcolor2' align=\"left\"><strong>" . $lang_new[$module_name]['LINK_ID'] . ": </td><td>$lid</strong></td></tr>\n";
echo "<tr><td bgcolor='$bgcolor2' align=\"left\">". $lang_new[$module_name]['LINK_SUBMITTER'] . ": </td><td>$modifysubmitter</td></tr>\n";
...
if ( !empty( $image ) ) {
    echo "<tr><td bgcolor='$bgcolor2' align=\"left\">". $lang_new[$module_name]['ADMIN_IMAGE_PREVIEW'] ."</td><td><img src=\"$image\" /></td></tr>\n";
}
...
echo Make_TextArea('description',$description, 'linkseditbroken');
echo "</td></tr>\n";
...
$result = $db->sql_query("SELECT `cid`, `title`, `parentid` FROM `"._WEBLINKS_CATEGORIES_TABLE."` ORDER BY `title`");
