...
$totalrating1   = 0;
$totalrating2   = 0;
$voteresult     = $db->sql_query("SELECT `rating`, `ratinguser` FROM `"._DOWNLOADS_VOTEDATA_TABLE."` WHERE `ratingdid` = '".$did."'");
$truecomments   = $db->sql_unumrows("SELECT `ratingdbid` FROM `"._DOWNLOADS_VOTEDATA_TABLE."` WHERE `ratingdid` = '".$did."' AND LENGTH(`ratingcomments`) > 1 ");
...
$outsideweight = round(100 / ($downloadsconfig['outsideweight'] ? $downloadsconfig['outsideweight'] : 100), 0);
$anonweight    = round(100 / ($downloadsconfig['anonweight'] ? $downloadsconfig['anonweight'] : 100), 0);
if ($totalvotes == 0) {
    $finalrating = 0;
} else {
