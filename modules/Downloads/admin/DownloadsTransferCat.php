...
OpenTable();
$numrows = $db->sql_numrows($db->sql_query("SELECT `cid` FROM `"._DOWNLOADS_DOWNLOADS_TABLE."`"));
if ($numrows>0) {
    ...
    $result = $db->sql_query("SELECT `cid`, `title`, `parentid` FROM `"._DOWNLOADS_CATEGORIES_TABLE."` ORDER BY `parentid`, `title`");
