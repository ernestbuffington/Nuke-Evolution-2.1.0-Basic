...
//if (is_user()||is_mod_admin($module_name)||) {
    ...
    if ($numresults > 0) {
        ...
        $row = $db->sql_fetchrow($db->sql_query("SELECT `cid`, `title`, `url`, `description` FROM `"._DOWNLOADS_DOWNLOADS_TABLE."` WHERE `did`='$did' AND `download_active`=1"));
        $cid = intval($row['cid']);
        ...
        echo "<input type=\"hidden\" name=\"op\" value=\"brokendownloadS\" /><input type=\"submit\" value=\"".$lang_new[$module_name]['DO_REPORT_BROKEN']."\" /></center></form>";
    }
    CloseTable();
