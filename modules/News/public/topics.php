...
function ModuleNewsTopicsNewIndex() {
    global $module_name, $actualtime, $_GETVAR, $neconfig, $userinfo, $querylang, $display, $storynum;

    $topic  = $_GETVAR->get('topic', '_REQUEST', 'int', 0);
    ...
    $db->sql_uquery('UPDATE `'._TOPICS_TABLE.'` SET `counter`=`counter`+1 WHERE `topicid`="'.$topic.'"');
    $total = $db->sql_ufetchrow('SELECT COUNT(`sid`) AS `total` FROM `'._STORIES_TABLE.'` WHERE `topic`="'.$topic.'" '.$querylang.' AND time <= "'.$actualtime.'"');
    $totalarticles = $total['total'];
    $result = $db->sql_query('SELECT `sid` FROM `'._STORIES_TABLE.'` WHERE `topic`="'.$topic.'" '.$querylang.' AND time <= "'.$actualtime.'" ORDER BY `catid`, `sid` DESC LIMIT '.$min.','.$storynum.'');
            ...
            while ($counter <= $articlepages ) {
                ...
                if ($counter == $currentpage) {
                    ...
                    if(defined('HOME_FILE')) {
                      ...
                      echo "<option value='modules.php?name=".$module_name."&amp;op=topics&amp;topic=".$topic."&amp;min=".$mintemp."'>".$counter."</option>\n";
                    }
                }
                $counter++;
            }
...
switch ($mode) {
    case 'newindex':
    default: ModuleNewsTopicsNewIndex(); break;
}
