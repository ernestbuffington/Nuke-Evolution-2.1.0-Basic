...
global $_GETVAR, $evoconfig, $supporter_config;
...
if($supporter_config['image_type']==0) {
    $site_image     = $_GETVAR->get('site_image', 'POST');
    $imageurl_name  = '';
    $imageurl_temp  = '';
    $site_image_type = 0;
} else {
    $site_image     = $_GETVAR->get('site_image', '_FILES', 'array');
    $imageurl_name  = $site_image['name'];
    $imageurl_temp  = $site_image['tmp_name'];
    $imageurl_type  = $site_image['Type'];
    $imageurl_error = $site_image['error'];
    $site_image_type = 1;
}
...
if($supporter_config['require_user'] == 0 || is_user()) {
    ...
    $result     = $db->sql_query("INSERT INTO `"._NSNSP_SITES_TABLE."` (`site_id`, `site_name`, `site_url`, `site_image`, `image_type`, `site_status`, `site_hits`, `site_date`, `site_description`, `user_id`, `user_name`, `user_email`, `user_ip`, `last_edit_user_date`, `last_edit_user_name`, `last_edit_user_id`)  
                                VALUES (NULL, '$site_name', '$site_url', '$imgurl', '$site_image_type', '0', '0', ".time().", '$site_description', '$user_id', '$user_name', '$user_email', '$user_ip', '0', '', '')");
    if(!$result) {
        include_once(NUKE_BASE_DIR.'header.php');
        OpenTable();
        ...
        $msg = EVO_SERVER_SITENAME .$lang_new[$module_name]['SP_ADDED']."\n\n";
        $msg .= $lang_new[$module_name]['SP_NAME'].": ".Remove_Slashes($site_name)."\n";
        $msg .= $lang_new[$module_name]['SP_URL'].": ".Remove_Slashes($site_url)."\n";
        $msg .= $lang_new[$module_name]['SP_IMAGE'].": ".Remove_Slashes($imgurl)."\n";
        $msg .= $lang_new[$module_name]['SP_DESCRIPTION'].": ".Remove_Slashes($site_description)."\n";
        $msg .= $lang_new[$module_name]['SP_USERID'].": $user_id\n";
        $msg .= $lang_new[$module_name]['SP_USERNAME'].": ".Remove_Slashes($user_name)."\n";
        $msg .= $lang_new[$module_name]['SP_USEREMAIL'].": ".Remove_Slashes($user_email)."\n";
        $msg .= $lang_new[$module_name]['SP_USERIP'].": $user_ip\n";
        $to = $evoconfig['adminmail'];
        $subject = EVO_SERVER_SITENAME." - ".$lang_new[$module_name]['SP_ADDED'];
        $return  = evo_mail($to, $subject, $msg);
