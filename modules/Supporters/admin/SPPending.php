...
$pagetitle = ': '.$lang_new[$module_name]['SP_ADMINMAIN'].' - '.$lang_new[$module_name]['SP_SUBMITTEDSITES'];
...
CloseTable();
echo "<br />";
title($lang_new[$module_name]['SP_ADMINMAIN'].' - '.$lang_new[$module_name]['SP_SUBMITTEDSITES']);
spmenu();
echo "<br />\n";
OpenTable();
$a = 0;
$result = $db->sql_query("SELECT * FROM `"._NSNSP_SITES_TABLE."` WHERE `site_status`='0' ORDER BY `site_name`");
...
if($numrows > 0) {
    ...
    while($site_row = $db->sql_fetchrow($result)) {
        ...
        echo "</td>\n<td width='75%' valign='top'><strong>".$lang_new[$module_name]['SP_USERNAME'].":</strong> ".$site_row['user_name']."</td></tr>";
        echo "<tr><td valign='top'><strong>".$lang_new[$module_name]['SP_DESCRIPTION']."</strong>: ".$cdescription."</td></tr>";
        echo "<tr><td align='center' valign='top'>";
        echo " <a href='".$admin_file.".php?op=SPApprove&amp;site_id=".$site_row['site_id']."&amp;comefrom=SPPending'><img src='".evo_image('approve.png', $module_name)."' width='16' height='16' border='0' alt='".$lang_new[$module_name]['SP_APPROVE']."' title='".$lang_new[$module_name]['SP_APPROVE']."' /></a>";
        echo " <a href='".$admin_file.".php?op=SPEdit&amp;site_id=".$site_row['site_id']."&amp;comefrom=SPPending'><img src='".evo_image('edit.png', $module_name)."' width='16' height='16' border='0' alt='"._EDIT."' title='"._EDIT."' /></a>";
        echo " <a href='".$admin_file.".php?op=SPDelete&amp;site_id=".$site_row['site_id']."&amp;comefrom=SPPending'><img src='".evo_image('delete.png', $module_name)."' width='16' height='16' border='0' alt='"._DELETE."' title='"._DELETE."' /></a>";
        echo "</td></tr>";
        echo "</table>";
        CloseTable();
        echo "</td>";
    ...
    echo "<center class='title'>".$lang_new[$module_name]['SP_NOSUBMITTEDSITES']."</center>\n";
}
CloseTable();
