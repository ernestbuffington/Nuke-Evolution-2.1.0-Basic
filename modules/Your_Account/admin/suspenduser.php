...
if(is_mod_admin($module_name)) {
    ...
    list($hisname) = $db->sql_fetchrow($db->sql_query("SELECT username FROM "._USERS_TABLE." WHERE user_id='$chng_uid'"));
    ...
    echo "<input type='hidden' name='file' value='suspendUserConf' />\n";
    echo "<input type='hidden' name='sus_uid' value='$chng_uid' />\n";
    echo "<tr><td align=center>"._SURE2SUSPEND." <strong>$hisname<em>($chng_uid)</em></strong>?</td></tr>\n";
    if ($evoconfig["servermail"] == 0) {
        echo "<tr><td align=center>"._SUSPENDREASON."<br /><textarea name='suspendreason' rows='5' cols='40' wrap='virtual'></textarea></td></tr>\n";
    }
