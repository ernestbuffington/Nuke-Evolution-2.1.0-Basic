...
if(is_mod_admin($module_name)) {
    global $db, $admin_file, $evoconfig, $board_config, $_GETVAR;
    
    $sus_uid  = $_GETVAR->get('sus_uid', '_REQUEST', 'int');
    $query    = $_GETVAR->get('chng_uid', '_REQUEST', 'string');
    $min      = $_GETVAR->get('chng_uid', '_REQUEST', 'int');
    $xop      = $_GETVAR->get('chng_uid', '_REQUEST', 'string');
    list($email, $username) = $db->sql_fetchrow($db->sql_query("SELECT user_email, username FROM "._USERS_TABLE." WHERE user_id='$sus_uid'"));
    if ($evoconfig['servermail']) {
        $message = _SORRYTO." ".EVO_SERVER_SITENAME." "._HASSUSPEND;
        if ($suspendreason > '') {
