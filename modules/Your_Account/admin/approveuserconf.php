...
if(is_mod_admin($module_name)) {
    ...
    $apr_uid  = $_GETVAR->get('apr_uid', '_REQUEST', 'int');
    ...
    list($username, $email, $check_num) = $db->sql_fetchrow($db->sql_query("SELECT username, user_email, check_num FROM "._USERS_TEMP_TABLE." WHERE user_id='$apr_uid'"));
    if ($evoconfig['servermail']) {
        $time     = time();
        $finishlink  = "<a href=\"".EVO_SERVER_URL."/modules.php?name=Profile&amp;mode=register&amp;agreed=true&amp;check_num=$check_num\">"._YA_ACTIVATE_CLICK."</a><br />( ".EVO_SERVER_URL."/modules.php?name=Profile&amp;mode=register&amp;agreed=true&amp;check_num=$check_num )";
    ...
    $db->sql_uquery("UPDATE "._USERS_TEMP_TABLE." SET time='$time' WHERE user_id='$apr_uid'");
