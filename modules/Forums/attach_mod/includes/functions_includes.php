...
function attachment_quota_settings($admin_mode, $submit = false, $mode) {
    global $template, $db, $HTTP_POST_VARS, $HTTP_GET_VARS, $lang, $lang, $phpbb_root_path, $attach_config;
...
function display_upload_attach_box_limits($user_id, $group_id = 0) {
    global $attach_config, $board_config, $phpbb_root_path, $lang, $db, $template, $userdata, $profiledata, $images;
    ...
    $template->assign_vars(array(
        'L_UACP'                => (is_admin() ? $lang['UACP'] : ''),
        'L_UPLOAD_QUOTA'        => $lang['Upload_quota'],
        'U_UACP'                => (is_admin() ? append_sid('uacp.php?u=' . $user_id . '&amp;sid=' . $userdata['session_id']) : ''),
