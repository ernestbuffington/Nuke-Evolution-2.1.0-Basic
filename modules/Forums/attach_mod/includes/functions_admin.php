...
function sort_multi_array ($sort_array, $key, $sort_order, $pre_string_sort = 0) {
    ...
    if (!$pre_string_sort) {
        $string_sort = ((isset($sort_array[$last_element-1][$key])) && (!is_numeric($sort_array[$last_element-1][$key]) ) ? true : false);
    } else {
    ...
    if (!intval($attach_config['allow_ftp_upload'])) {
        if ($dir = @opendir($upload_dir)) {
            while ($file = @readdir($dir)) {
                if ($file != 'index.html' && $file != 'index.php' && $file != '.htaccess' && !is_dir($upload_dir . '/' . $file) && !is_link($upload_dir . '/' . $file)) {
        ...
        for ($i = 0; $i < sizeof($file_listing); $i++) {
            if (preg_match("#([-d])[rwxst-]{9}.* ([0-9]*) ([a-zA-Z]+[0-9: ]*[0-9]) ([0-9]{2}:[0-9]{2}) (.+)#si", $file_listing[$i], $regs)) {
                if ($regs[1] == 'd') {
