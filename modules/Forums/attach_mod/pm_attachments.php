...
class attach_pm extends attach_parent
{
    ...
    */
    function insert_attachment_pm($a_privmsgs_id)
    {
        ...
        if ($a_privmsgs_id && ($mode == 'post' || $mode == 'masspm' || $mode == 'reply' || $mode == 'edit') && intval($attach_config['allow_pm_attach']))
        {
    ...
    */
    ...
    {
        global $attach_config, $template, $lang, $userdata, $HTTP_POST_VARS, $phpbb_root_path, $db;
        global $confirm, $delete, $delete_all, $group_id, $post_id, $privmsgs_id, $privmsg_id, $submit, $refresh, $mark_list, $folder;
    ...
    if (($this->pm_delete_attachments || $delete) && sizeof($mark_list))
        {
            if (!$userdata['session_logged_in'])
            {
                ...
                redirect(append_sid("login.php?redirect=privmsg&amp;folder=inbox", true));
                exit;
            }
