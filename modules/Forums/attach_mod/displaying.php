...
function display_review_attachments($post_id, $switch_attachment, $is_auth){
    ...
    $is_auth['auth_download'] = (isset($is_auth['auth_download']) ? $is_auth['auth_download'] : FALSE);
    $is_auth['auth_view']     = (isset($is_auth['auth_view']) ? $is_auth['auth_view'] : FALSE);
...
function init_display_review_attachments($is_auth){
    ...
    if (!isset($is_auth['auth_download']) || $is_auth['auth_download'] == TRUE) {
        $is_authdl = TRUE;
    } else {
        $is_authdl = FALSE;
    }
    if (!isset($is_auth['auth_view']) || $is_auth['auth_view'] == TRUE) {
        $is_authvw = TRUE;
    } else {
        $is_authvw = FALSE;
    }
    if (intval($attach_config['disable_mod']) || !($is_authdl && $is_authvw) || intval($attach_config['attachment_topic_review']) == 0) {
        return;
    }
    init_display_template('reviewbody', '{attachrow.ATTACHMENTS}');
    init_complete_extensions_data();
}
...
function display_attachments_preview($attachment_list, $attachment_filesize_list, $attachment_filename_list, $attachment_comment_list, $attachment_extension_list, $attachment_thumbnail_list){
    global $attach_config, $is_auth, $allowed_extensions, $lang, $userdata, $display_categories, $upload_dir, $upload_icons, $template, $db, $theme, $evoconfig;
    ...
    if (sizeof($attachment_list) != 0) {
        ...
        for ($i = 0, $size = sizeof($attachment_list); $i < $size; $i++)  {
            ...
            if (!$denied)  {
                ...
                if ($image) {
                    // Images
                    if (($height && ($height > $evoconfig['img_height'])) || ($width && ($width > $evoconfig['img_width']))) {
                    ...
                    $template->assign_block_vars('postrow.attach.cat_images', array(
                        ...
                        'FILESIZE'              => $filesize,
                        'SIZE_VAR'              => $size_lang,
                        'IMG_WIDTH'             => $new_dimensions['width'],
                        'IMG_HEIGHT'            => $new_dimensions['height'],
                        'COMMENT'               => $comment,
                        'L_DOWNLOADED_VIEWED'   => $lang['Viewed'])
                    );
                }
                if ($thumbnail) {
                    list($width, $height) = image_getdimension(NUKE_BASE_DIR.$thumb_filename);
                    if (($height && ($height > $evoconfig['img_height'])) || ($width && ($width > $evoconfig['img_width']))) {
                        $new_dimensions = evo_img_resize_dimensions($evoconfig['img_width'],$evoconfig['img_height'], $width, $height);
                    } else {
                        $new_dimensions['width']  = $width;
...
function display_attachments($post_id) {
    ...
    global $phpbb_root_path, $evoconfig;
    ...
    for ($i = 0; $i < $num_attachments; $i++) {
        ...
        if (!$denied || $userdata['user_level'] == ADMIN) {
            ...
            if ($image) {
                ...
                if (($height && ($height > $evoconfig['img_height'])) || ($width && ($width > $evoconfig['img_width']))) {
                    $new_dimensions = evo_img_resize_dimensions($evoconfig['img_width'],$evoconfig['img_height'], $width, $height);
                    $template->assign_block_vars('switch_lightbox', array());
                } else {
                    $new_dimensions['width']  = $width;
                    $new_dimensions['height'] = $height;
                }
                ...
                $template->assign_block_vars('postrow.attach.cat_images', array(
                    ...
                    'COMMENT'               => $comment,
                    'L_DOWNLOADED_VIEWED'   => $lang['Viewed'],
                    'L_DOWNLOAD_COUNT'      => sprintf($lang['Download_number'], ($attachments['_' . $post_id][$i]['download_count'] + 1)))
                );
            ...
            if ($thumbnail) {
                ...
                if (intval($attach_config['allow_ftp_upload']) && trim($attach_config['download_path']) == '') {
                    ...
                    if ($attach_config['upload_dir'][0] == '/' || ( $attach_config['upload_dir'][0] != '/' && $attach_config['upload_dir'][1] == ':')) {
                        ...
                        list($width, $height) = image_getdimension($thumb_source);
                       // END
                    }
                }
                if (($height && ($height > $evoconfig['img_height'])) || ($width && ($width > $evoconfig['img_width']))) {
                    $new_dimensions = evo_img_resize_dimensions($evoconfig['img_width'],$evoconfig['img_height'], $width, $height);
                    $template->assign_block_vars('postrow.attach.cat_thumb_images.switch_lightbox', array());
                } else {
                    $new_dimensions['width']  = $width;
                    $new_dimensions['height'] = $height;
                }
