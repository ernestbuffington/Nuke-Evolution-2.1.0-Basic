...
class Stats_template {
    ...
    function set_template($stats_template = '') {
        global $directory_mode, $phpbb_root_path, $theme_name;
        $stats_template = $theme_name;
        $this->root = NUKE_STATS_DIR . $stats_template . '/forums/stats_mod';
    ...
    function compile($code, $do_not_echo = false, $retvar = '') {
        ...
        for($i = 0, $max = count($text_blocks);$i < $max; $i++) {
            $this->compile_var_tags($text_blocks[$i]);
        }
        $compile_blocks = array();
        for ($curr_tb = 0; $curr_tb < count($text_blocks); $curr_tb++) {
            switch ((isset($blocks[1][$curr_tb]) ? $blocks[1][$curr_tb] : '')) {
                case 'BEGIN':
