...
include($phpbb_root_path . 'stats_mod/content/bars.php');
include($phpbb_root_path . 'stats_mod/content/statistical.php');
include($phpbb_root_path . 'stats_mod/content/values.php');

// db cache
include($phpbb_root_path . 'stats_mod/db_cache.php');
include($phpbb_root_path . 'stats_mod/functions.php');
...
class StatisticsCORE {
    ...
    global $board_config;
        ...
        foreach ($this->calculation_data as $key) {
            if (!empty($key['topic_title']) && $board_config['smilies_in_titles']) {
                $this->calculation_data[$i]['topic_title'] = smilies_pass($board_config['smilies_in_titles']) ? smilies_pass($key['topic_title']) : $key['topic_title'];
            }
            $i++;
        }
    }
    ...
    function run_module() {
        ...
        $template->assign_block_vars('modules', array(
            ...
            'MODULE_NAME'               => $this->current_module_name,
    ...
    function set_content($content_template) {
        ...
        if ($content_template == 'bars') {
            ...
            $content->init_bars(array(
                'left' => evo_image('leftbar.png', 'Statistics'),
                'right' => evo_image('rightbar.png', 'Statistics'),
                'bar' => evo_image('mainbar.png', 'Statistics'))
            );
