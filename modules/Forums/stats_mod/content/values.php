        ...
        for ($i = 0; $i < count($core->calculation_data); $i = $i + $this->num_blocks) {
            ...
            for ($j = 0; $j < $this->num_blocks; $j++) {
                for ($k = 0; $k < $this->columns; $k++) {
                    if ($this->value_order == 'up_down') {
                        $row_value[$num] = (isset($core->calculation_data[$iteration+($one_block*$j)][$k]) ? $core->calculation_data[$iteration+($one_block*$j)][$k] : 0);
                    } else if ($this->value_order == 'left_right') {
                        $row_value[$num] = (isset($core->calculation_data[$j+($iteration*$one_block)][$k]) ? $core->calculation_data[$j+($iteration*$one_block)][$k] : 0);
                    }
                    $num++;
                }
            }
