...
class Content_bars {
    ...
    function init_bars($bars = '') {
        global $board_config, $db, $stats_template;
        ...
        if ($this->bar_loaded) {
            $stats_template->assign_vars(array(
                'LEFT_GRAPH_IMAGE'  => $this->loaded_bar_images['left'],
                'RIGHT_GRAPH_IMAGE' => $this->loaded_bar_images['right'],
                'GRAPH_IMAGE'       => $this->loaded_bar_images['bar'])
            );
            return;
        }
        $stats_template->assign_vars(array(
    ...
    function set_rows($data) {
        global $core, $stats_template, $stat_functions, $lang;
        ...
        for ($i = 0; $i < count($core->calculation_data); $i++) {
            ...
            for ($j = 0; $j < $this->columns; $j++) {
                ...
                $stats_template->assign_block_vars('row.row_column', array(
                    ...
                    'ROW'               => $i,
                    'VALUE'             => (isset($row_value[$j]) ? $row_value[$j] : 0),
                    'ALIGNMENT'         => $this->align[$j],
                    'AUTH_REPLACEMENT'  => (isset($auth_replace[$j]['replace']) ? $auth_replace[$j]['replace'] : TRUE),
                    'AUTH_LANG_ENTRY'   => (isset($auth_replace[$j]['lang']) ? $auth_replace[$j]['lang'] : TRUE))
                );
            }
            $core->calc_index++;
        }
