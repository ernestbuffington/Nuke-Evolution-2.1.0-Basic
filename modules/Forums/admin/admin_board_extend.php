...
if (defined('PHPBB_BOARD_CONFIG')) {
    if( !empty($setmodules) ) {
        ...
        $module['General']['Configuration_extend'] = $filename;
        return;
    }
}
...
$var_menu       = $_GETVAR->get('menu', 'get', 'int');
$var_menu_id    = $_GETVAR->get('menu_id', 'post', 'int');
$var_mod        = $_GETVAR->get('mod', 'get', 'int');
$var_mod_id     = $_GETVAR->get('mod_id', 'post', 'int');
$var_msub       = $_GETVAR->get('sub_id', 'get', 'int');
$var_sub_id     = $_GETVAR->get('sub_id', 'post', 'int');
...
//
$mods = array();
$dir = @opendir(NUKE_FORUMS_ADMIN_DIR.'mods_settings');
while( $file = @readdir($dir) ) {
    if( preg_match("/^mod_.*?\.php$/", $file) ) {
        include_once(NUKE_FORUMS_ADMIN_DIR.'mods_settings/' . $file);
    }
}
@closedir($dir);
...
// menu_id
$menu_id = 0;
if ( isset($var_menu) || isset($var_menu_id) ) {
    $menu_id = isset($var_menu_id) ? intval($var_menu_id) : intval($var_menu);
}
// mod_id
$mod_id = 0;
if ( isset($var_mod) || isset($var_mod_id) ) {
    $mod_id = isset($var_mod_id) ? intval($var_mod_id) : intval($var_mod);
}
// sub_id
$sub_id = 0;
if ( isset($var_msub) || isset($var_sub_id) ) {
    $sub_id = isset($var_sub_id) ? intval($var_sub_id) : intval($var_msub);
}
...
// sub name
$sub_name = (isset($sub_keys[$menu_id][$mod_id][$sub_id]) ? $sub_keys[$menu_id][$mod_id][$sub_id] : '');

// buttons
$submit = isset($var_submit);
...
if ($submit) {
    ...
    while ( list($field_name, $field) = @each($mods[$menu_name]['data'][$mod_name]['data'][$sub_name]['data']) ) {
            switch ($field['type']) {
                case 'LIST_RADIO':
                case 'LIST_DROP':
                   $$field_name = $_GETVAR->get($field_name, 'post', 'int');
                ...
                case 'TEXT':
                case 'DATEFMT':
                    $$field_name = trim(str_replace("\'", "''", htmlspecialchars($_GETVAR->get($field_name, 'post', 'string'))));
                    break;
                case 'HTMLVARCHAR':
                case 'HTMLTEXT':
                    $$field_name = trim(str_replace("\'", "''", $_GETVAR->get($field_name, 'post', 'string')));
                    break;
                default:
                    $$field_name = '';
                    if ( !empty($field['chk_func']) && function_exists($field['chk_func']) ) {
                        $$field_name = $field['chk_func']($field_name, $_GETVAR->get($field_name, 'post'));
                    } else {
            ...
            if ($error) {
                $message = $error_msg . '<br /><br />' . sprintf($lang['Click_return_config'], '<a href="' . admin_sid("admin_board_extend.php&amp;menu=$menu_id&amp;mod=$mod_id&amp;msub=$sub_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . admin_sid("index.php&amp;pane=right") . '">', '</a>');
    ...
    while ( list($field_name, $field) = @each($mods[$menu_name]['data'][$mod_name]['data'][$sub_name]['data']) ) {
        ...
        if ( $_GETVAR->get($field_name.'over', 'post', 'int', NULL) && !empty($field['user']) && isset($userdata[ $field['user'] ]) ) {
            // update
            $sql = "UPDATE " . CONFIG_TABLE . "
                    SET config_value = '" . $_GETVAR->get($field_name.'over', 'post', 'int') . "'
    ...
    $message = $lang['Config_updated'] . '<br /><br />' . sprintf($lang['Click_return_config'], '<a href="' . admin_sid("admin_board_extend.php&amp;menu=$menu_id&amp;mod=$mod_id&amp;msub=$sub_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . admin_sid("index.php&amp;pane=right") . '">', '</a>');
    message_die(GENERAL_MESSAGE, $message);
}
...
for ($i = 0; $i < count($menu_keys); $i++) {
    ...
    $template->assign_block_vars('menu', array(
        ...
        'U_MENU'  => admin_sid('admin_board_extend.php&amp;menu='.$i),
        'L_MENU'  => sprintf( ( ($menu_id == $i) ? '<strong>%s</strong>' : '%s' ), mods_settings_get_lang($l_menu) ),
        )
    );
    if ( $menu_id == $i ) {
        if (count($mod_keys[$i]) > 1 ) {
    ...
    if ($menu_id == $i) {
        for ($j = 0; $j < count($mod_keys[$i]); $j++ ) {
            ...
            $template->assign_block_vars('menu.mod', array(
                ...
                'U_MOD' => admin_sid('admin_board_extend.php&amp;menu='.$i.'&amp;mod='.$j),
                'L_MOD' => sprintf( ( ( ($menu_id == $i) && ($mod_id == $j) ) ? '<strong>%s</strong>' : '%s' ), mods_settings_get_lang($l_mod) ),
                )
            );
                ...
                if ( count($sub_keys[$i][$j]) > 1 ) {
                    ...
                    for ($k = 0; $k < count($sub_keys[$i][$j]); $k++) {
                        $template->assign_block_vars('menu.mod.sub.row', array(
                            ...
                            'U_MOD' => admin_sid('admin_board_extend.php&amp;menu='.$i.'&amp;mod='.$j.'&amp;msub='.$k),
                            'L_MOD' => sprintf( (($sub_id == $k) ? '<strong>%s</strong>' : '%s'), mods_settings_get_lang($sub_keys[$i][$j][$k]) ),
                            )
                        );
                    }
                }
...
$template->assign_vars(array(
    'S_ACTION'      => admin_sid('admin_board_extend.php'),
