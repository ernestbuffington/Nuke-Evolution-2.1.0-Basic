...
if (defined('PHPBB_BOARD_CONFIG')) {
    if( !empty($setmodules) ) {
        ...
        $module['AUC']['Configuration']     = $filename;
        return;
    }
}
...
$link = admin_sid('admin_advanced_username_color.php');

function move_up($group) {
    global $db, $lang, $cache, $link;

    #==== Get current weight & weight above it to switch them.
    $group_one = $db->sql_ufetchrow("SELECT * FROM ". AUC_TABLE ." WHERE group_id = '". $group ."'", SQL_ASSOC);
    if ($group_one['group_weight'] < 2) {
        redirect($link); // nothing to do
    } else {
        $above_it  = $group_one['group_weight'] - 1;
        $group_two = $db->sql_ufetchrow("SELECT * FROM ". AUC_TABLE ." WHERE group_weight = '". $above_it ."'", SQL_ASSOC);
        $group_one_id        = $group_one['group_id'];
        $group_one_weight    = $group_one['group_weight'];
        $group_one_color     = $group_one['group_color'];
        $group_two_id        = $group_two['group_id'];
        $group_two_weight    = $group_two['group_weight'];
        #==== Set new settings for the groups
        $db->sql_uquery("UPDATE ". AUC_TABLE ." SET group_weight = '". $group_two_weight ."' WHERE group_id = '". $group_one_id ."'");
        $db->sql_uquery("UPDATE ". AUC_TABLE ." SET group_weight = '". $group_one_weight ."' WHERE group_id = '". $group_two_id ."'");
        //OK we have changed the priority of the Colors.
        //Get all users which have this color
        $users = $db->sql_query("SELECT `user_id` FROM `".USERS_TABLE."`
                                WHERE `user_color_gi` REGEXP '".$group_one_id."'");
        //Now we have all users which are involved in this move for the colorgroup which got a higher priority
        //So let's update the user-colors for those ones
        while ($row = $db->sql_fetchrow($users)) {
             add_group_attributes($row['user_id'], $group_one_id, 0, 1, 1, 0, 1);
        }
...
function ColorTable() {
  $color_table = "<table width='100%' cellspacing='0' cellpadding='2' border='0' align='center' class='forumline'>\n
...
function main() {
    ...
    echo "        </th>";
    echo "    </tr>";
    $q = "SELECT * FROM ". AUC_TABLE ." WHERE group_id > '0'";
    $r        = $db->sql_query($q);
    ...
    while($row = $db -> sql_fetchrow($r)) {
        if ($w != 1) {
            $links = '&nbsp;&nbsp;<a href="'.admin_sid('admin_advanced_username_color.php?mode=move_up&amp;id='. $row['group_id'] .'&amp;sid='. $userdata['session_id']) .'">'. $lang['move_up'] .'</a>';
        } else {
            $links = '';
        }
        echo "    <tr>";
    ...
    $q = "SELECT * FROM ". AUC_TABLE ." WHERE group_id > '0' ORDER BY group_name ASC";
    $r = $db -> sql_query($q);
    while($row = $db -> sql_fetchrow($r)) {
    ...
    echo "</form>";
    echo "<br /><br />";
    $show_table = ColorTable();
    echo $show_table;
}

function del_exis_group($to_delete) {
    global $db, $lang, $cache;

    $list_user = '';
    $changed_users = '<br /><div style="text-align:left;"><span style="font-weight:bold;text-align:center;">&nbsp;</span>';
    $actual_color  = $db->sql_ufetchrow('SELECT `group_color` FROM `'. AUC_TABLE .'` WHERE `group_id` = "'. $to_delete .'"');
    $db->sql_uquery('DELETE FROM `'. AUC_TABLE .'` WHERE `group_id` = "'. $to_delete .'"');
    $db->sql_uquery('UPDATE `'. GROUPS_TABLE .'` SET `group_color` = "" WHERE `group_color` = "'. $to_delete .'"');
    // Now we have to reorder the group_weight
    $query = 'SELECT `group_id` FROM `'. AUC_TABLE .'` ORDER BY `group_weight`';
    $result = $db->sql_query($query);
    $count = 1;
    while ( $sorting = $db->sql_fetchrow($result) ) {
        $db->sql_query('UPDATE `'. AUC_TABLE .'` SET `group_weight` = ' . $count . ' WHERE `group_id` = ' . $sorting['group_id']);
        $count++;
    }
    $db->sql_freeresult($result);
    //OK we have changed the priority of the Colors.
    //Get all users which have this color
    $users = $db->sql_query("SELECT `user_id`, `username` FROM `".USERS_TABLE."`
                            WHERE `user_color_gi` REGEXP '".$to_delete."'");
    //Now we have all users which are involved in this colorgroup delete which have eventually to be changed
    //So let's update the user-colors for those ones
    while ($row = $db->sql_fetchrow($users)) {
        $return = add_group_attributes($row['user_id'], $to_delete, 0, 1, 1, 1, 0);
        if ($return) {
            $list_user .= $row['username'].'<br >';
        }
    }
    $db->sql_uquery('UPDATE `'.GROUPS_TABLE.'` SET `group_color` = "" WHERE `group_color`="'.$to_delete.'"');
    $db->sql_freeresult($users);
    ...
    message_die(GENERAL_MESSAGE, $lang['delete_success'] . "<br />" . $changed_users . "<br />" . sprintf($lang['Return_to_config'], "<a href=".admin_sid('admin_advanced_username_color.php').">", "</a></div>"), $lang['success']);
}

function edit_exis_group($group) {
    global $db, $lang, $link;
    $q = "SELECT * FROM ". AUC_TABLE ." WHERE group_id = '". $group ."'";
    $r = $db->sql_query($q);
...
function save_new_color($new_name, $new_color, $old_name, $group) {
    global $db, $lang, $cache;
    if(strlen($new_color) <> 6) {
        message_die(GENERAL_ERROR, $lang['add_error_3'] . "<br /><br />" . sprintf($lang['Return_to_config'], "<a href=".admin_sid('admin_advanced_username_color.php').">", "</a>"), $lang['error']);
    }
    $q = "SELECT group_name FROM ". AUC_TABLE ." WHERE group_name = '". $new_name ."'";
    $r = $db -> sql_query($q);
    $row    = $db -> sql_fetchrow($r);
    ...
    if($new_name != $old_name) {
        if($exists == $new_name) {
            message_die(GENERAL_ERROR, $lang['save_error_1'] . "<br /><br />" . sprintf($lang['Return_to_config'], "<a href=".admin_sid('admin_advanced_username_color.php').">", "</a>"), $lang['error']);
        }
    }
    $row = $db->sql_ufetchrow("SELECT group_color FROM ". AUC_TABLE ." WHERE group_id = '". $group ."'");
    $current_c  = $row['group_color'];
    $db->sql_uquery("UPDATE ". AUC_TABLE ." SET group_name = '". $new_name ."', group_color = '". $new_color ."' WHERE group_id = '". $group ."'");
    $db->sql_uquery ("UPDATE ". USERS_TABLE ." SET user_color_gc = '". $new_color ."' WHERE user_color_gc = '". $current_c ."' AND user_color_gi REGEXP '--". $group . "--'");
    $cache->delete('UserColors', 'config');
    $cache->delete('GroupColors', 'config');
    message_die(GENERAL_MESSAGE, $lang['add_success'] . "<br /><br />" . sprintf($lang['Return_to_config'], "<a href=".admin_sid('admin_advanced_username_color.php').">", "</a>"), $lang['success']);
}

function add_new_color($color_name, $color_color) {
    global $db, $lang;
    if(strlen($color_color) <> 6) {
        message_die(GENERAL_ERROR, $lang['add_error_3'] . "<br /><br />" . sprintf($lang['Return_to_config'], "<a href=".admin_sid('admin_advanced_username_color.php').">", "</a>"), $lang['error']);
    }
    $q = "SELECT group_name FROM ". AUC_TABLE ." WHERE group_name = '". $color_name ."'";
    $r = $db -> sql_query($q);
    $row = $db -> sql_fetchrow($r);
    $db->sql_freeresult($r);
    if($row['group_name']) {
        message_die(GENERAL_ERROR, $lang['add_error_2'] . "<br /><br />" . sprintf($lang['Return_to_config'], "<a href=".admin_sid('admin_advanced_username_color.php').">", "</a>"), $lang['error']);
    }
    $q = "SELECT MAX(group_weight) AS max_weight FROM ". AUC_TABLE ."";
    $r = $db -> sql_query($q);
    $next = $db -> sql_fetchrow($r);
    $db->sql_freeresult($r);
    $weight = $next['max_weight'] + 1;
    $db->sql_uquery("INSERT INTO ". AUC_TABLE ." (`group_id`, `group_name`, `group_color`, `group_weight`) VALUES ('', '". $color_name ."', '". $color_color ."', '". $weight ."')");
    message_die(GENERAL_MESSAGE, $lang['add_success'] . "<br /><br />" . sprintf($lang['Return_to_config'], "<a href=".admin_sid('admin_advanced_username_color.php').">", "</a>"), $lang['success']);
}

$modus          = $_GETVAR->get('mode', 'request', 'string', 'main');
$group_id       = $_GETVAR->get('id', 'request', 'int', NULL);
$group          = $_GETVAR->get('group', 'post', 'int', NULL);
$new_name       = $_GETVAR->get('new_name', 'post', 'string', NULL);
$new_color      = $_GETVAR->get('new_color', 'post', 'string', NULL);
$old_name       = $_GETVAR->get('old_name', 'post', 'string', '');
$color_name     = $_GETVAR->get('new_name', 'post', 'string', NULL);
$color_color    = $_GETVAR->get('new_color', 'post', 'string', NULL);

switch ($modus) {
  case 'add_new_color':
      add_new_color($color_name, $color_color);
      break;
  case 'save_new_color':
      save_new_color($new_name, $new_color, $old_name, $group_id);
      break;
  case 'edit_exis_group':
      edit_exis_group($group);
      break;
  case 'del_exis_group':
      del_exis_group($group);
      break;
  case 'move_up':
      move_up($group_id);
      break;
