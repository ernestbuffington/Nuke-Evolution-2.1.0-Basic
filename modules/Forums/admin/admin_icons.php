...
if (defined('PHPBB_BOARD_CONFIG')) {
    if( !empty($setmodules) ) {
        ...
        $module['General']['Icons_settings'] = $filename;
        return;
    }
}
...
function icons_write() {
    global  $template, $icones, $icon_defined_special, $map_icon;
    ...
    // output the file
    $filename = NUKE_FORUMS_DIR . 'post_icon_mod/includes/def_icons.php';
    @CHMOD($filename, 0666);
    @unlink($filename);
    $f = @fopen($filename, 'w' );
    $texte  = "<?php\n$res\n?>";
...
if (!in_array($mode, array('edit', 'up', 'dw', 'del'))) {
    $mode = '';
}

// icon
$icon = $_GETVAR->get('icon', 'request', 'int', -1);

// buttons
$create   = $_GETVAR->get('create', 'request', 'string', NULL);
$submit   = $_GETVAR->get('submit', 'request', 'string', NULL);
$confirm  = $_GETVAR->get('confirm', 'request', 'string', NULL);
$cancel   = $_GETVAR->get('cancel', 'request', 'string', NULL);
$refresh  = $_GETVAR->get('refresh', 'request', 'string', NULL);
    ...
    if ($cancel) {
        ...
        if (isset($_POST['replace_icon'])) {
            ...
            if (isset($map_icon[$replace_icon])) {
                ...
                if (!$db->sql_query($sql)) {
                    message_die(GENERAL_ERROR, 'unable to update icon on posts', '', __LINE__, __FILE__, $sql);
                }
                // replace topic icons
                $sql = "UPDATE " . TOPICS_TABLE . " SET topic_icon=$replace_icon WHERE topic_icon=$icon";
                if (!$db->sql_query($sql)) {
                    message_die(GENERAL_ERROR, 'unable to update icon on topics', '', __LINE__, __FILE__, $sql);
                }
            }
        }
        // update the table
    ...
    if (isset($map_icon[$icon])) {
        ...
        if (!$used) {
            ...
            if (!$result = $db->sql_query($sql)) {
                message_die(GENERAL_ERROR, 'unable to access posts', '', __LINE__, __FILE__, $sql);
            }
        ...
        if (!$used) {
            ...
            if (!$result = $db->sql_query($sql)) {
                message_die(GENERAL_ERROR, 'unable to access topics', '', __LINE__, __FILE__, $sql);
            }
            $used = ($row = $db->sql_fetchrow($result));
        }
        // some prevent check
        ...
        $template->assign_vars(array(
            ...
            'S_ACTION'        => admin_sid('admin_icons.php'),
...
if ($mode == 'edit') {
    ...
    // read the formular
    if (isset($_POST['icon_title'])) {
        $icon_title   = trim(str_replace("\'", "''", $_POST['icon_title']));
    }
    if (isset($_POST['icon_url'])) {
        $icon_url   = trim(str_replace("\'", "''", $_POST['icon_url']));
    }
    if (isset($_POST['icon_auth'])) {
        $icon_auth    = trim(str_replace("\'", "''", $_POST['icon_auth']));
    }
    if ($refresh || $submit) {
        $icon_ids = array();
        $icon_ids = $_POST['ids'];
    ...
    if ($mode == 'edit') {
        ...
        $pic = '';
        if (!empty($url)) {
            $pic = '<img name="icon_image" src="' . $url . '" align="middle" alt="' . (isset($lang[$icon_title]) ? $lang[$icon_title] : '') . '" border="0" />&nbsp;';
        }
        ...
        $s_icons = '<select name="icon_url_pickup_list" onchange="javascript:document.icon_url.value=this.options[this.selectedIndex].value; this.selectedIndex=0;">' . $s_icons . '</select>';
        // lang keys list
        ...
        $s_langs = '<select name="lang_key_pickup_list" onchange="javascript:document.icon_title.value=this.options[this.selectedIndex].value; this.selectedIndex=0;document.">' . $s_langs . '</select>';
        // vars
        $template->assign_vars(array(
...
if ($mode == '') {
    ...
    $sql = "SELECT post_icon, count(*) as count FROM " . POSTS_TABLE . "
            ...
            ORDER BY post_icon";
    if (!$result = $db->sql_query($sql)) {
        message_die(GENERAL_ERROR, 'unable to count icons on posts', '', __LINE__, __FILE__, $sql);
    }
    $total_posts = 0;
    while ($row = $db->sql_fetchrow($result)) {
        $total_posts = $total_posts + $row['count'];
        $row_icon = intval($row['post_icon']);
        if (isset($map_icon[$row_icon])) {
            $icones[$map_icon[$row_icon]]['usage'] = (isset($icones[$map_icon[$row_icon]]['usage']) ? $icones[$map_icon[$row_icon]]['usage'] : 0) + $row['count'];
        }
    }
    if ($total_posts <= 0) {
        $total_posts = 1;
    ...
    for ($i=0; $i < count($icones); $i++) {
        $template->assign_block_vars('row', array(
            ...
            'USAGE'     => (isset($icones[$i]['usage']) && (intval($icones[$i]['usage']) > 0)) ? $icones[$i]['usage'] . '&nbsp;(' . ( round( ($icones[$i]['usage'] * 100 )/ $total_posts ) ) . '%)' : '',
            'U_EDIT'    => admin_sid('admin_icons.php&amp;mode=edit&amp;icon=' . $icones[$i]['ind']),
            'U_DELETE'  => admin_sid('admin_icons.php&amp;mode=del&amp;icon=' . $icones[$i]['ind']),
            'U_MOVEUP'  => admin_sid('admin_icons.php&amp;mode=up&amp;icon=' . $icones[$i]['ind']),
            'U_MOVEDW'  => admin_sid('admin_icons.php&amp;mode=dw&amp;icon=' . $icones[$i]['ind']),
            )
        );
