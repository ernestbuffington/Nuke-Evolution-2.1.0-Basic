...
if ($submit && $mode == 'extensions') {
    ...
    if ($extension_id_sql != '') {
        $sql = 'DELETE FROM ' . EXTENSIONS_TABLE . ' WHERE ext_id IN (' . $extension_id_sql . ')';
        if (!$result = $db->sql_query($sql)) {
    ...
    $add = ( $_GETVAR->get('add_extension_check', 'post', 'string') ) ? TRUE : FALSE;
    if ($extension != '' && $add) {
    ...
    if (!$error) {
        $message = $lang['Attach_config_updated'] . '<br /><br />' . sprintf($lang['Click_return_attach_config'], '<a href="' . admin_sid("admin_extensions.php&amp;mode=extensions") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . admin_sid("index.php&amp;pane=right") . '">', '</a>');
...
if ($mode == 'extensions') {
    ...
    $template->assign_vars(array(
        ...
        'S_CANCEL_ACTION'       => admin_sid('admin_extensions.php&amp;mode=extensions'),
        'S_ATTACH_ACTION'       => admin_sid('admin_extensions.php&amp;mode=extensions'))
    );
    if ($submit) {
        $template->assign_vars(array(
...
if ($submit && $mode == 'groups') {
    ...
    if ($group_id_sql != '') {
        $sql = 'DELETE FROM ' . EXTENSION_GROUPS_TABLE . ' WHERE group_id IN (' . $group_id_sql . ')';
        if (!($result = $db->sql_query($sql))) {
    ...
    $is_allowed         = ($_GETVAR->get('add_allowed', 'post', 'string')) ? 1 : 0;
    $add                = ($_GETVAR->get('add_extension_group_check', 'post', 'string') ) ? TRUE : FALSE;
    if ($extension_group != '' && $add) {
        // check Extension Group
        $sql = 'SELECT group_name FROM ' . EXTENSION_GROUPS_TABLE;
        if (!($result = $db->sql_query($sql))) {
    ...
    if (!$error) {
        $message = $lang['Attach_config_updated'] . '<br /><br />' . sprintf($lang['Click_return_attach_config'], '<a href="' . admin_sid("admin_extensions.php&amp;mode=groups") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . admin_sid("index.php&amp;pane=right") . '">', '</a>');
        message_die(GENERAL_MESSAGE, $message);
    }
}
...
if ($mode == 'groups') {
    ...
    $template->assign_vars(array(
        ...
        'S_CANCEL_ACTION'           => admin_sid('admin_extensions.php&amp;mode=groups'),
        'S_ATTACH_ACTION'           => admin_sid('admin_extensions.php&amp;mode=groups'))
    );
    ...
    for ($i = 0; $i < $num_extension_group; $i++) {
        ...
        $template->assign_block_vars('grouprow', array(
            ...
            'U_VIEWGROUP'        => ($viewgroup == $extension_group[$i]['group_id']) ? admin_sid('admin_extensions.php&amp;mode=groups') : admin_sid('admin_extensions.php?mode=groups&amp;' . POST_GROUPS_URL . '=' . $extension_group[$i]['group_id']),
            'U_FORUM_PERMISSIONS'=> admin_sid('admin_extensions.php&amp;mode=' . $mode . '&amp;e_mode=perm&amp;e_group=' . $extension_group[$i]['group_id']))
        );
        if ($viewgroup && $viewgroup == $extension_group[$i]['group_id']) {
            $sql = 'SELECT comment, extension FROM ' . EXTENSIONS_TABLE . ' WHERE group_id = ' . (int) $viewgroup;
            if (!$result = $db->sql_query($sql)) {
                message_die(GENERAL_ERROR, 'Couldn\'t get Extension informations', '', __LINE__, __FILE__, $sql);
            }
...
if ($submit && $mode == 'forbidden') {
    ...
    if ($extension_id_sql != '') {
        $sql = 'DELETE FROM ' . FORBIDDEN_EXTENSIONS_TABLE . '
    ...
    if (!$error) {
        $message = $lang['Attach_config_updated'] . '<br /><br />' . sprintf($lang['Click_return_attach_config'], '<a href="' . admin_sid("admin_extensions.php&amp;mode=forbidden") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . admin_sid("index.php&amp;pane=right") . '">', '</a>');
        message_die(GENERAL_MESSAGE, $message);
    }
}
...
if ($mode == 'forbidden') {
    ...
    $template->assign_vars(array(
        'S_ATTACH_ACTION'       => admin_sid('admin_extensions.php&amp;mode=forbidden'),
    ...
    $sql = 'SELECT * FROM ' . FORBIDDEN_EXTENSIONS_TABLE . ' ORDER BY extension';
    if (!($result = $db->sql_query($sql))) {
...
if ($e_mode == 'perm') {
    ...
    $add_forum      = ($_GETVAR->get('add_forum', 'post', 'string')) ? TRUE : FALSE;
    $delete_forum   = ($_GETVAR->get('del_forum', 'post', 'string')) ? TRUE : FALSE;
    if ($_GETVAR->get('close_perm', 'post', 'string')) {
        $e_mode = '';
    }
} else {
    $add_forum = FALSE;
...
if ($e_mode == 'perm' && $group) {
    ...
    $template->assign_vars(array(
        ...
        'L_RESET'                     => $lang['Reset'],
        'A_PERM_ACTION'               => admin_sid('admin_extensions.php&amp;mode=groups&amp;e_mode=perm&amp;e_group='.$group))
    );
