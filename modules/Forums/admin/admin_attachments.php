...
global $directory_mode, $_GETVAR, $board_config;
...
include_once(NUKE_FORUMS_DIR . 'attach_mod/includes/functions_selects.php');
include_once(NUKE_FORUMS_DIR . 'attach_mod/includes/functions_admin.php');

if (!intval($attach_config['allow_ftp_upload'])) {
    if ( ($attach_config['upload_dir'][0] == '/') || ( ($attach_config['upload_dir'][0] != '/') && ($attach_config['upload_dir'][1] == ':') ) ) {
        $upload_dir = $attach_config['upload_dir'];
    } else {
        if (!$attach_config['upload_dir'][0] == '') {
...
$e_mode             = $_GETVAR->get('e_mode', 'request');
$size               = $_GETVAR->get('size', 'request', 'string');
$quota_size         = $_GETVAR->get('quota_size', 'request', 'string');
$pm_size            = $_GETVAR->get('pm_size', 'request', 'string');
$var_search_imagick = $_GETVAR->get('search_imagick', 'post', 'int');
$var_submit         = $_GETVAR->get('submit', 'post', 'string');
$var_settings       = $_GETVAR->get('settings', 'post', 'string');
$var_cat_settings   = $_GETVAR->get('cat_settings', 'post', 'string');
$submit             = (isset($var_submit)) ? TRUE : FALSE;
$check_upload       = (isset($var_settings)) ? TRUE : FALSE;
$check_image_cat    = (isset($var_cat_settings)) ? TRUE : FALSE;
$search_imagick     = (isset($var_search_imagick)) ? TRUE : FALSE;
$error = false;
...
while ($row = $db->sql_fetchrow($result)) {
    ...
    $new_attach[$config_name] = $_GETVAR->get($config_name, 'post', 'string', trim($attach_config[$config_name]));
...
$cache_dir  = NUKE_FORUMS_DIR . 'cache';
...
if ($check_upload) {
    ...
    if (!$error) {
        message_die(GENERAL_MESSAGE, $lang['Test_settings_successful'] . '<br /><br />' . sprintf($lang['Click_return_attach_config'], '<a href="' . admin_sid("admin_attachments.php&amp;mode=manage") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . admin_sid("index.php?pane=right") . '">', '</a>'));
    }
}
...
if ($submit && $mode == 'manage') {
    if (!$error) {
        message_die(GENERAL_MESSAGE, $lang['Attach_config_updated'] . '<br /><br />' . sprintf($lang['Click_return_attach_config'], '<a href="' . admin_sid("admin_attachments.php&amp;mode=manage") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . admin_sid("index.php?pane=right") . '">', '</a>'));
    }
}
...
if ($mode == 'manage') {
    ...
    $template->assign_vars(array(
        ...
        'S_ATTACH_ACTION'               => admin_sid('admin_attachments.php&amp;mode=manage'),
...
if ($submit && $mode == 'shadow') {
    ...
    $attach_file_list = $_GETVAR->get('attach_file_list', 'post', 'array');
    ...
    $attach_id_list = $_GETVAR->get('attach_id_list', 'post', 'array');
    ...
    $message = $lang['Attach_config_updated'] . '<br /><br />' . sprintf($lang['Click_return_attach_config'], '<a href="' . admin_sid("admin_attachments.php&amp;mode=shadow") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . admin_sid("index.php?pane=right") . '">', '</a>');
...
if ($mode == 'shadow') {
    ...
    $template->assign_vars(array(
        ...
        'S_ATTACH_ACTION'   => admin_sid('admin_attachments.php&amp;mode=shadow'))
    );
    ...
    $count_tableattachments   = (isset($table_attachments) ? count($table_attachments) : 0);
    $count_fileattachments    = (isset($file_attachments) ? count($file_attachments) : 0);
    $count_assigndattachments = (isset($assign_attachments) ? count($assign_attachments) : 0);
    $cs = 0;
    $ca = 0;
    for ($i = 0; $i < $count_fileattachments; $i++) {
        if ($count_tableattachments > 0) {
            if ($file_attachments[$i] != '') {
                if (!in_array(trim($file_attachments[$i]), $table_attachments['physical_filename']) ) {
                    $shadow_attachments[$ca] = trim($file_attachments[$i]);
    ...
    for ($i = 0; $i < $count_assigndattachments; $i++)  {
        if (!in_array($assign_attachments[$i], $table_attachments['attach_id'])) {
            $shadow_row['attach_id'][$cs] = $assign_attachments[$i];
            $shadow_row['physical_filename'][$cs] = $assign_attachments[$i];
            $shadow_row['comment'][$cs] = $lang['Empty_file_entry'];
            $cs++;
        }
    }
    // Go through the Database and get those Files not stored at the Filespace
    for ($i = 0; $i < $count_tableattachments; $i++) {
        if ($table_attachments['physical_filename'][$i] != '') {
            if ( !in_array(trim($table_attachments['physical_filename'][$i]), $file_attachments)) {
                $shadow_row['attach_id'][$cs] = $table_attachments['attach_id'][$i];
                $shadow_row['physical_filename'][$cs] = trim($table_attachments['physical_filename'][$i]);
                $shadow_row['comment'][$cs] = $table_attachments['comment'][$i];
...
if ($submit && $mode == 'cats') {
    if (!$error) {
        message_die(GENERAL_MESSAGE, $lang['Attach_config_updated'] . '<br /><br />' . sprintf($lang['Click_return_attach_config'], '<a href="' . admin_sid("admin_attachments.php&amp;mode=cats") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . admin_sid("index.php?pane=right") . '">', '</a>'));
    }
}
...
if ($mode == 'cats') {
    ...
    $template->assign_vars(array(
        ...
        'S_ATTACH_ACTION'               => admin_sid('admin_attachments.php&amp;mode=cats'))
    );
}
...
if ($check_image_cat) {
    ...
    if ($attach_config['upload_dir'][0] == '/' || ($attach_config['upload_dir'][0] != '/' && $attach_config['upload_dir'][1] == ':')) {
        ...
        $upload_dir = NUKE_FORUMS_DIR . $attach_config['upload_dir'];
    }
    ...
    if (!$error) {
        message_die(GENERAL_MESSAGE, $lang['Test_settings_successful'] . '<br /><br />' . sprintf($lang['Click_return_attach_config'], '<a href="' . admin_sid("admin_attachments.php&amp;mode=cats") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . admin_sid("index.php?pane=right") . '">', '</a>'));
    }
}
...
if ($submit && $mode == 'quota') {
    // Change Quota Limit
    $quota_change_list  = $_GETVAR->get('quota_change_list', 'post', 'array');
    $quota_desc_list    = $_GETVAR->get('quota_desc_list', 'post', 'array');
    $filesize_list      = $_GETVAR->get('max_filesize_list', 'post', 'array');
    $size_select_list   = $_GETVAR->get('size_select_list', 'post', 'array');
    $allowed_list       = array();
    ...
    // Delete Quota Limits
    $quota_id_list = $_GETVAR->get('quota_id_list', 'post', 'array', array());
    $quota_id_sql = implode(', ', $quota_id_list);
    if ($quota_id_sql != '') {
        $sql = 'DELETE FROM ' . QUOTA_LIMITS_TABLE . '
    ...
    // Add Quota Limit ?
    $quota_desc     = $_GETVAR->get('quota_description', 'post');
    $filesize       = $_GETVAR->get('add_max_filesize', 'post', 'int');
    $size_select    = $_GETVAR->get('add_size_select', 'post');
    $add            = ( $_GETVAR->get('add_quota_check', '_POST') ) ? TRUE : FALSE;

    if ($quota_desc != '' && $add) {
        // check Quota Description
        $sql = 'SELECT quota_desc FROM ' . QUOTA_LIMITS_TABLE;
        if (!($result = $db->sql_query($sql))) {
    ...
    if (!$error) {
        $message = $lang['Attach_config_updated'] . '<br /><br />' . sprintf($lang['Click_return_attach_config'], '<a href="' . admin_sid("admin_attachments.php&amp;mode=quota") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . admin_sid("index.php?pane=right") . '">', '</a>');
        message_die(GENERAL_MESSAGE, $message);
    }
}
...
if ($mode == 'quota') {
    ...
    $template->assign_vars(array(
        ...
        'S_ATTACH_ACTION'         => admin_sid('admin_attachments.php&amp;mode=quota'))
    );
    ...
    for ($i = 0; $i < sizeof($rows); $i++) {
        ...
        $template->assign_block_vars('limit_row', array(
            ...
            'U_VIEW'       => admin_sid("admin_attachments.php&amp;mode=$mode&amp;e_mode=view_quota&amp;quota_id=" . $rows[$i]['quota_limit_id']),
...
if ($mode == 'quota' && $e_mode == 'view_quota') {
    $quota_id = $_GETVAR->get('quota_id', 'request', 'int');
    if (!$quota_id) {
