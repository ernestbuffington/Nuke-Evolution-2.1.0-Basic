...
global $_GETVAR;
require_once(NUKE_INCLUDE_DIR . 'functions_search.php');
require_once(NUKE_INCLUDE_DIR . 'functions_admin_rebuild_search.php');

//
// Language File
//
$lang_file = '/lang_admin_rebuild_search.php';
...
$modus = $_GETVAR->get('mode', 'request', 'string', '');
// check if we should process in fast mode
$fast_mode = $_GETVAR->get('fast_mode', 'request', 'string', NULL) ? TRUE : FALSE;
...
if ( $mode == 'refresh' ) {
    $disable_board = $_GETVAR->get('disable_board', 'request', 'int', 0);
} else if ( $mode == 'submit' ) {
    // when pressing submit, check 'disabled_board_status' parameter and then 'disable_board' checkbox
    $disable_board = $_GETVAR->get('disabled_board_status', 'request', 'int', NULL) ? ( $disable_board = 2 ) :  ($_GETVAR->get('disable_board', 'request', 'int') ? 1 : 0 );
} else {
    // when starting the mod, check the admin config
...
if (  $_GETVAR->get('cancel_button', 'request', 'string', NULL) ) {
    ...
    $message =  sprintf($lang['Rebuild_search_aborted'], $end_post_id).'<br /><br />'.sprintf($lang['Click_return_rebuild_search'], '<a href="'.admin_sid("admin_rebuild_search.php").'">', '</a>');
    message_die(GENERAL_MESSAGE, $message);
}

// from which post to start processing
$start = $_GETVAR->get('start', 'request', 'int', -1);
// clear the search tables or not
$clear_search = $_GETVAR->get('clear_search', 'request', 'int', 0);
...
if (  $_GETVAR->get('session_posts_processing', 'request', 'int', NULL) ) {
    $session_posts_processing =   $_GETVAR->get('session_posts_processing', 'request', 'int');
    if ( $mode == 'submit' ) {
...
$post_limit = $_GETVAR->get('post_limit', 'request', 'int', $def_post_limit);
...
if ( $_GETVAR->get('time_limit', 'request', 'int', NULL) ) {
    $time_limit = $_GETVAR->get('time_limit', 'request', 'int');
} else {
    ...
    if ( $_GETVAR->get('HTTP_KEEP_ALIVE', 'server', 'int', NULL) ) {
        // get webserver timeout
        $webserver_timeout = $_GETVAR->get('HTTP_KEEP_ALIVE', 'server', 'int');
...
$refresh_rate = $_GETVAR->get('refresh_rate', 'request', 'int', $def_refresh_rate);
// check if the user gave wrong input
if ( $mode == 'submit' ) {
    if ( $session_posts_processing <= 0 || $post_limit <= 0 || $refresh_rate < 0 || $time_limit <=0 ) {
        $message =  $lang['Wrong_input'].'<br /><br />'.sprintf($lang['Click_return_rebuild_search'], '<a href="'.admin_sid("admin_rebuild_search.php").'">', '</a>');
        message_die(GENERAL_MESSAGE, $message);
    }
}
//
...
if ( $mode == 'submit' || $mode == 'refresh' ) {
    ...
    if ( $session_posts_processed < $session_posts_processing && $total_posts_processed < $total_posts ) {
        ...
        $form_action = admin_sid('admin_rebuild_search.php&amp;mode=refresh'.$form_parameters);
        $next_button = $lang['Rebuild_Search_Next'];
        ...
        $form_action = admin_sid('admin_rebuild_search.php');
    ...
    $page_title = $lang['Rebuild_search'];
    ...
    $template->assign_vars(array(
        ...
        'S_REBUILD_SEARCH_ACTION'   => admin_sid("admin_rebuild_search.php&amp;mode=submit" . ( ( $disable_board == 2 ) ? ('&amp;disabled_board_status=' . $disable_board) : ''))
        )
    );
}
