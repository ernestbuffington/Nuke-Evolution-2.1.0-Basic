...
$mode       = get_var('mode', '');
$view       = get_var('view', '');
$var_u_id   = $_GETVAR->get('u_id', 'post', 'int');
$var_submit_change = $_GETVAR->get('submit_change', 'post', 'string');
$var_delete = $_GETVAR->get('delete', 'post', 'string');
$var_search = $_GETVAR->get('search', 'post', 'string');
$var_confirm = $_GETVAR->get('confirm', 'post', 'string');
$var_start  = $_GETVAR->get('start', 'get', 'int');
$uid        = isset($var_u_id) ? get_var('u_id', 0) : get_var('uid', 0);
$view       = (isset($var_search) && $var_search) ? 'attachments' : $view;
$total_attachments = 0;
...
$submit_change = isset($var_submit_change) ? TRUE : FALSE;
$delete = isset($var_delete) ? TRUE : FALSE;
$delete_id_list = get_var('delete_id_list', array(0));

$confirm = ($var_confirm) ? TRUE : FALSE;
...
if ($confirm && sizeof($delete_id_list) > 0) {
    ...
    $template->assign_vars(array(
        ...
        'L_YES'                 => $lang['Yes'],
        'L_NO'                  => $lang['No'],
        'S_CONFIRM_ACTION'      => admin_sid('admin_attach_cp.php'),
...
$template->assign_vars(array(
    ...
    'S_MODE_ACTION'             => admin_sid('admin_attach_cp.php'))
);
    ...
    if ($num_members > 0) {
        ...
        for ($i = 0; $i < count($members); $i++) {
            ...
            $template->assign_block_vars('memberrow', array(
                'ROW_NUMBER'        => $i + ( $var_start + 1 ),
                'ROW_COLOR'         => '#' . $row_color,
                'ROW_CLASS'         => $row_class,
                'USERNAME'          => $username,
                'TOTAL_ATTACHMENTS' => $total_attachments,
                'TOTAL_SIZE'        => round(($total_size / MEGABYTE), 2),
                'U_VIEW_MEMBER'     => admin_sid('admin_attach_cp.php&amp;view=attachments&amp;uid=' . $members[$i]['user_id']))
            );
        }
    }
    $sql = "SELECT user_id_1
...
if ($view == 'attachments') {
    ...
    $search_based = (isset($var_search) && $var_search) ? TRUE : FALSE;
    $hidden_fields = '';
    $template->set_filenames(array(
    ...
    for ($i = 0; $i < sizeof($attachments); $i++) {
            ...
            for ($j = 0; $j < $num_ids; $j++) {
                if ($ids[$j]['post_id'] != 0) {
                    ...
                    $view_topic = append_sid('viewtopic.php?' . POST_POST_URL . '=' . $ids[$j]['post_id'] . '&menu=1#' . $ids[$j]['post_id']);
            ...
            $template->assign_block_vars('attachrow', array(
                ...
                'U_VIEW_ATTACHMENT'    => append_sid('download.php?id=' . $attachments[$i]['attach_id'] . '&amp;menu=1'))
            );
        }
    }
