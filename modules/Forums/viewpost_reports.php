...
if( !is_user() ) {
    redirect('login.php?redirect=viewpost_reports.php');
}
...
$status     = $_GETVAR->get('status', 'request', 'string', REPORT_POST_NEW);
$mode       = $_GETVAR->get('mode', 'request');
$report_id  = $_GETVAR->get('report', 'request', 'int', -1);

// check for single open/close report
if ( ($mode == 'closereport' || $mode == 'openreport') && $report_id != -1 ) {
    // maybe we have to add a check here if the report is already closed/opened
    $last_action_comments = trim($_GETVAR->get('last_action_comments', 'request', 'string', ''));
    ...
    if ( empty($last_action_comments) ) {
        ...
        $template->assign_vars(array(
            ...
            'U_VIEW_TOPIC'                  => append_sid('viewtopic.php?' . POST_POST_URL . '=' . $post_id . '#' . $post_id),
            ...
            'S_ACTION'                      => append_sid('viewpost_reports.php?mode='.$mode.'&amp;report='.$report_id))
        );
        ...
        $message =  $lang['Close_success'] . '<br /><br />' . sprintf($lang['Click_return_reports'], '<a href="' . append_sid('viewpost_reports.php') . '">', '</a>');
    ...
    $reports = $_GETVAR->get(POST_POST_URL, 'post', 'array', array());
    if ( empty($reports) ) {
        $message =  $lang['Report_not_selected'] . '<br /><br />' . sprintf($lang['Click_return_reports'], '<a href="' . append_sid('viewpost_reports.php') . '">', '</a>');
        message_die(GENERAL_MESSAGE, $message);
    }
    $report_ids = array();
    ...
    $message = $lang['Opt_success'] . '<br /><br />' . sprintf($lang['Click_return_reports'], '<a href="' . append_sid('viewpost_reports.php') . '">', '</a>');
    message_die(GENERAL_MESSAGE, $message);
} else if ( $mode == 'delete' ) {
    ...
    $message =  $lang['Delete_success'] . '<br /><br />' . sprintf($lang['Click_return_reports'], '<a href="' . append_sid('viewpost_reports.php') . '">', '</a>');
    message_die(GENERAL_MESSAGE, $message);
}
...
$template->assign_vars(array(
    ...
    'U_OPT_OUT'             => ( $userdata['user_report_optout'] ) ? append_sid('viewpost_reports.php?mode=optin') : append_sid('viewpost_reports.php?mode=optout'),
    ...
    'S_ACTION'              => append_sid('viewpost_reports.php'))
);
show_reports($status);
