...
$querylang = ($evoconfig['multilingual'] == 1) ? "WHERE planguage='$currentlang' AND artid='0'" : "WHERE artid='0'";
...
if ($db->sql_numrows($result) < 1) {
    ...
    if ($db->sql_numrows($result) > 0) {
        ...
        if (isset($mainbar_d)) $m1_size = @getimagesize(evo_image_dir('mainbar.png', 'Surveys'));
    ...
    <span class=\"content\"><a href=\"modules.php?name=Surveys&amp;op=results&amp;pollID=$pollID&amp;mode=".$userinfo['umode']."&amp;order=".$userinfo['uorder']."&amp;thold=".$userinfo['thold']."\"><strong>"._RESULTS."</strong></a><br />
    <a href=\"modules.php?name=Surveys\"><strong>"._POLLS."</strong></a><br />
    <br />"._VOTES." <strong>$sum</strong>\n";
    if ($evoconfig['pollcomm']) {
        list($numcom) = $db->sql_fetchrow($db->sql_query("SELECT COUNT(tid) FROM "._POLL_COMMENTS_TABLE." WHERE pollID='".$pollID."'"));
