...
$totalrating1   = 0;
$totalrating2   = 0;
$voteresult     = $db->sql_query("SELECT `rating`, `ratinguser` FROM `"._REVIEWS_VOTEDATA_TABLE."` WHERE `ratingrid` = '".$rid."'");
$truecomments   = $db->sql_unumrows("SELECT `ratingdbid` FROM `"._REVIEWS_VOTEDATA_TABLE."` WHERE `ratingrid` = '".$rid."' AND LENGTH(`ratingcomments`) > 1 ");
...
$outsideweight = round(100 / ($reviewsconfig['outsideweight'] ? $reviewsconfig['outsideweight'] : 100), 0);
$anonweight    = round(100 / ($reviewsconfig['anonweight'] ? $reviewsconfig['anonweight'] : 100), 0);
if ($totalvotes == 0) {
    $finalrating = 0;
} else {
