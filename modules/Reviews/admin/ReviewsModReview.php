...
echo "<form method=\"post\" action=\"".$admin_file.".php\" name=\"reviewsmodreview\">";
echo "<table width=\"100%\" border=\"0\">\n";
$result = $db->sql_query("SELECT `cid`, `title`, `image`, `url`, `description`, `name`, `email`, `hits`, `date`, `date_validated`, `header`, `footer`, `body`, `signature` FROM `"._REVIEWS_REVIEWS_TABLE."` WHERE `rid`='$rid'");
while($row = $db->sql_fetchrow($result)) {
    ...
    echo "<tr><td bgcolor='$bgcolor2' align=\"left\">". $lang_new[$module_name]['REVIEW_VALIDATE_DATE'] . ": </td><td>".(($date_validated >0 ) ? formatTimestamp($date_validated) : 0)."</td></tr>";
    ...
    echo "<center>[ <a href=\"".$admin_file.".php?op=ReviewsDelReview&amp;rid=$rid\">" . $lang_new[$module_name]['SUBMIT_DELETE'] . "</a> ]</center>\n";
    echo "</form>";
    echo "<br />";
    /* Modify or Add Editorial */
    $resulted2 = $db->sql_query("SELECT `adminid`, `editorialtimestamp`, `editorialtext`, `editorialtitle` FROM `"._REVIEWS_EDITORIALS_TABLE."` WHERE `reviewid`='$rid'");
    ...
    if ($recordexist == 0) {
        ...
        echo "<table width=\"100%\" border=\"0\">\n";
        echo "<input type=\"hidden\" name=\"reviewid\" value=\"$rid\" />";
        ...
        echo "<tr><td bgcolor='$bgcolor2' align=\"left\">". $lang_new[$module_name]['DESCRIPTION'] . ": </td><td>";
        echo Make_TextArea('editorialtext',$editorialtext, 'modifyadd', '100%', '100px');
        echo "</td></tr>\n";
        echo "</table><br />";
        echo "<input type=\"hidden\" name=\"op\" value=\"ReviewsAddEditorial\" />\n";
        ...
        while($row3 = $db->sql_fetchrow($resulted2)) {
            ...
            echo "<table width=\"100%\" border=\"0\">\n";
            echo "<tr><td align=\"left\">". $lang_new[$module_name]['AUTHOR'] . ": </td><td>$adminid</td></tr>\n";
            echo "<tr><td align=\"left\">". $lang_new[$module_name]['DATE_WRITTEN'] . ": </td><td>$formatted_date</td></tr>\n";
            echo "<input type=\"hidden\" name=\"reviewid\" value=\"$rid\" />\n";
            echo "<tr><td align=\"left\">". $lang_new[$module_name]['TITLE'] . ": </td><td><input type=\"text\" name=\"editorialtitle\" value=\"$editorialtitle\" size=\"75\" maxlength=\"100\" /></td></tr>";
            ...
            echo "</td></tr>\n";
            echo "</table><br />";
            echo "<input type=\"hidden\" name=\"op\" value=\"ReviewsModEditorial\" />\n";
            echo "<center><input type=\"submit\" value=\"" . $lang_new[$module_name]['SUBMIT_MODIFY'] . "\" /></center>\n";
            echo "<center>[ <a href=\"".$admin_file.".php?op=ReviewsDelEditorial&amp;reviewid=$rid\">" . $lang_new[$module_name]['SUBMIT_DELETE'] . "</a> ]</center>\n";
            echo "</form>";
            CloseTable();
        }
    ...
    echo "<br />";
    /* Show Comments */
    $temp_color = $db->sql_query("SELECT `config_name`, `config_value` FROM `". _REVIEWS_CONFIG_TABLE ."` WHERE `config_name` LIKE 'tablecolor%'");
    ...
    $result4 = $db->sql_query("SELECT `ratingdbid`, `ratinguser`, `ratingcomments`, `ratingtimestamp` FROM `"._REVIEWS_VOTEDATA_TABLE."` WHERE `ratingrid` = '$rid' AND `ratingcomments` <> '' ORDER BY `ratingtimestamp` DESC");
    ...
    while($row4 = $db->sql_fetchrow($result4)) {
        ...
        echo "<tr><td valign=top bgcolor=\"$colorswitch\">$ratinguser</td><td valign=\"top\" colspan=\"5\" bgcolor=\"$colorswitch\">$ratingcomments</td><td bgcolor=\"$colorswitch\"><center><strong><a href=".$admin_file.".php?op=ReviewsDelComment&amp;rid=$rid&amp;rrdbid=$ratingdbid>X</a></strong></center></td></tr>\n";
        $x++;
    ...
    $result5 = $db->sql_query("SELECT `ratingdbid`, `ratinguser`, `rating`, `ratinghostname`, `ratingtimestamp` FROM `"._REVIEWS_VOTEDATA_TABLE."` WHERE `ratingrid` = '$rid' AND `ratinguser` <> 'outside' AND `ratinguser` <> '' ORDER BY `ratingtimestamp` DESC");
    $totalvotes = $db->sql_numrows($result5);
    echo "<br /><fieldset><legend><span class='option'><strong>" . $lang_new[$module_name]['ADMIN_REVIEW_VOTE_REGUSER'] . "</strong> (". $lang_new[$module_name]['ADMIN_REVIEW_VOTE_TOTAL'] .": $totalvotes)</span></legend>";
    echo "<table width=\"100%\" border=\"0\">\n";
    ...
    echo "<td width=\"10%\"  align=\"center\" bgcolor=\"$bgcolor1\"><strong>". $lang_new[$module_name]['ADMIN_REVIEW_RATING'] ."</strong></td>";
    echo "<td width=\"15%\"  align=\"center\" bgcolor=\"$bgcolor1\"><strong>". $lang_new[$module_name]['DATE'] ."</strong></td>";
    echo "<td width=\"10%\"  align=\"center\" bgcolor=\"$bgcolor1\"><strong>". $lang_new[$module_name]['ADMIN_REVIEW_RATING_TOTAL'] ."</strong></td>";
    echo "<td width=\"13%\"  align=\"center\" bgcolor=\"$bgcolor1\"><strong>". $lang_new[$module_name]['ADMIN_REVIEW_RATING_AVERAGE'] ."</strong></td>";
    ...
    while($row5 = $db->sql_fetchrow($result5)) {
        ...
        $result6 = $db->sql_query("SELECT `rating` FROM `"._REVIEWS_VOTEDATA_TABLE."` WHERE `ratinguser` = '$ratinguser'");
    ...
    $result7 = $db->sql_query("SELECT `ratingdbid`, `rating`, `ratinghostname`, `ratingtimestamp` FROM `"._REVIEWS_VOTEDATA_TABLE."` WHERE `ratingrid` = '$rid' AND `ratinguser` = '' ORDER BY `ratingtimestamp` DESC");
    $totalvotes = $db->sql_numrows($result7);
    echo "<br /><fieldset><legend><span class='option'><strong>" . $lang_new[$module_name]['ADMIN_REVIEW_VOTE_UNREG'] . "</strong> (". $lang_new[$module_name]['ADMIN_REVIEW_VOTE_TOTAL'] .": $totalvotes)</span></legend>";
    echo "<table width=\"100%\" border=\"0\">\n";
    echo "<tr><td bgcolor=\"$bgcolor1\" colspan=\"2\"><strong>" . $lang_new[$module_name]['IP_ADRESS'] ."</strong></td><td bgcolor=\"$bgcolor1\" colspan=\"3\"><strong>" . $lang_new[$module_name]['ADMIN_REVIEW_RATING'] ."</strong></td><td bgcolor=\"$bgcolor1\"><strong>". $lang_new[$module_name]['DATE'] ."</strong></td><td bgcolor=\"$bgcolor1\"><strong><center>". $lang_new[$module_name]['DELETE'] ."</center></strong></td></tr>\n";
    if ($totalvotes == 0) {
        echo "<tr><td colspan=\"7\"><center>". $lang_new[$module_name]['WARN_VOTE_NOT_FOUND'] ."</center></td></tr>\n";
    }
    $x=0;
    ...
    while($row7 = $db->sql_fetchrow($result7)) {
        ...
        echo "<td colspan=\"2\" bgcolor=\"$colorswitch\">$ratinghostname</td><td colspan=\"3\" bgcolor=\"$colorswitch\">$rating</td><td bgcolor=\"$colorswitch\">$formatted_date</td><td bgcolor=\"$colorswitch\"><center><strong><a href=".$admin_file.".php?op=ReviewsDelVote&amp;rid=$rid&amp;rrdbid=$ratingdbid>X</a></strong></center></td></tr>\n";
        $x++;
        if ($colorswitch == $color['tablecolor1']) {
              $colorswitch = $color['tablecolor2'];
        } else {
    ...
    $result8 = $db->sql_query("SELECT `ratingdbid`, `rating`, `ratinghostname`, `ratingtimestamp` FROM `"._REVIEWS_VOTEDATA_TABLE."` WHERE `ratingrid` = '$rid' AND `ratinguser` = 'outside' ORDER BY `ratingtimestamp` DESC");
    $totalvotes = $db->sql_numrows($result8);
    echo "<br /><fieldset><legend><span class='option'><strong>" . $lang_new[$module_name]['ADMIN_REVIEW_VOTE_GUESTS'] . "</strong> (". $lang_new[$module_name]['ADMIN_REVIEW_VOTE_TOTAL'] .": $totalvotes)</span></legend>";
    echo "<table width=\"100%\" border=\"0\">\n";
    echo "<tr><td bgcolor=\"$bgcolor1\" colspan=\"2\"><strong>". $lang_new[$module_name]['IP_ADRESS'] ."</strong></td><td bgcolor=\"$bgcolor1\" colspan=\"3\"><strong>". $lang_new[$module_name]['ADMIN_REVIEW_RATING'] ."</strong></td><td bgcolor=\"$bgcolor1\"><strong>". $lang_new[$module_name]['DATE'] ."</strong></td><td bgcolor=\"$bgcolor1\"><strong><center>". $lang_new[$module_name]['DELETE'] ."</center></strong></td></tr>\n";
    if ($totalvotes == 0) {
        echo "<tr><td colspan=\"7\"><center>". $lang_new[$module_name]['WARN_VOTE_NOT_FOUND'] ."</center></td></tr>\n";
    ...
    while($row8 = $db->sql_fetchrow($result8)) {
        ...
        echo "<tr><td colspan=\"2\" bgcolor=\"$colorswitch\">$ratinghostname</td><td colspan=\"3\" bgcolor=\"$colorswitch\">$rating</td><td bgcolor=\"$colorswitch\">$formatted_date</td><td bgcolor=\"$colorswitch\"><center><strong><a href=".$admin_file.".php?op=ReviewsDelVote&amp;rid=$rid&amp;rrdbid=$ratingdbid>X</a></strong></center></td></tr>\n";
        $x++;
        if ($colorswitch == $color['tablecolor1']) {
              $colorswitch = $color['tablecolor1'];
        } else {
