...
OpenTable();
$numrows = $db->sql_numrows($db->sql_query("SELECT `cid` FROM `"._REVIEWS_REVIEWS_TABLE."`"));
if ($numrows>0) {
    echo "<center><span class=\"option\"><strong>" . $lang_new[$module_name]['ADMIN_TRANSFER_CAT'] . "</strong></span></center><br /><br />\n";
    echo "<form method=\"post\" action=\"".$admin_file.".php\" name=\"reviewtransfercat\">";
    echo "<table width=\"100%\" border=\"0\">\n";
    echo "<tr><td align=\"left\">". $lang_new[$module_name]['CATEGORY'] . ": </td><td>";
    echo "<select name=\"cidfrom\">";
    $result = $db->sql_query("SELECT `cid`, `title`, `parentid` FROM `"._REVIEWS_CATEGORIES_TABLE."` ORDER BY `parentid`, `title`");
    ...
    echo "<tr><td align=\"left\">". $lang_new[$module_name]['CATEGORY'] . ": </td><td>";
    $result1 = $db->sql_query("SELECT `cid`, `title`, `parentid` FROM `"._REVIEWS_CATEGORIES_TABLE."` ORDER BY `parentid`, `title`");
    ...
    echo "</select></td></tr>\n";
    echo "</table><br />";
    echo "<input type=\"hidden\" name=\"op\" value=\"ReviewsTransfer\" />";
