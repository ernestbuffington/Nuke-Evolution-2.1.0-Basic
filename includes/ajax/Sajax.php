...
class Sajax {
    ...
    function sajax_handle_client_request() {
        $mode = (!empty($_GET['rs']) ? 'get' : (!empty($_POST['rs']) ? 'post' : false));
        if(!$mode) return;
    ...
    function sajax_add_script($script) {
        $this->sajax_added_scripts .= $script;
    }
