...
$rss->syndicationURL=EVO_SERVER_URL . '/rss.php?feed=downloads';
...
if (!empty($cat)) {
    $catid = $db->sql_fetchrow($db->sql_query("SELECT cid FROM "._DOWNLOADS_CATEGORIES_TABLE." WHERE title LIKE '%$cat%' LIMIT 1"));
    if (empty($catid)) {
        $result = $db->sql_query("SELECT did, title, description, date, submitter FROM "._DOWNLOADS_DOWNLOADS_TABLE." ORDER BY did DESC ".$num);
    } else {
        $catid = intval($catid);
        $result = $db->sql_query("SELECT did, title, description, date, submitter FROM "._DOWNLOADS_DOWNLOADS_TABLE." WHERE cid='$catid' ORDER BY did DESC ".$num);
    }
} else {
    $result = $db->sql_query("SELECT did, title, description, date, submitter FROM "._DOWNLOADS_DOWNLOADS_TABLE." ORDER BY did DESC ".$num);
}
...
while ($row = $db->sql_fetchrow($result)) {
    ...
    $item->link     = EVO_SERVER_URL."/modules.php?name=Downloads&amp;op=getit&amp;did=".$row['did'];
    ...
    $item->guid     = $row['did'].'@'.EVO_SERVER_URL;
    $item->date     = $row['date'];
    $rss->addItem($item);
}
