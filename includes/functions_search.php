...
function add_search_words($mode, $post_id, $post_text, $post_title = '') {
    ...
    if ( count($word) ) {
        ...
        if ( $value_sql != '' ) {
            $sql = "REPLACE INTO " . SEARCH_WORD_TABLE . " (word_text, word_common)
                    VALUES $value_sql";
            if ( !$db->sql_query($sql) ) {
    ...
    while( list($word_in, $match_sql) = @each($word_insert_sql) ) {
        ...
        if ( $match_sql != '' ) {
            $sql = "REPLACE INTO " . SEARCH_MATCH_TABLE . " (post_id, word_id, title_match)
...
// Username search
//
function username_search($search_match, $group_id=0) {
    global $db, $board_config, $template, $lang, $images, $theme, $phpbb_root_path, $starttime, $gen_simple_header;
    ...
    $template->assign_vars(array(
        ...
        'U_GROUP_ID'         => $group_id,
        'S_SEARCH_ACTION'    => append_sid('search.php?mode=searchuser&amp;popup=1&amp;group_id='.$group_id)
        )
    );
    if ( $username_list != '' ) {
        if ( $group_id != 0 ) {
            $template->assign_block_vars('switch_select_group', array());
        } else {
