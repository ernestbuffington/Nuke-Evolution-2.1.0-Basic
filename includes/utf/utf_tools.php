...
/*
* @package utf
* @version $Id: utf_tools.php,v 1.69 2007/08/16 12:19:26 naderman Exp $
...
function utf8_recode($string, $encoding)
{
  ...
  if (preg_match('/iso[_ -]?8859[_ -]?(\\d+)/', $encoding, $array))
  {
    switch ($array[1])
    {
      ...
      case '15':
        ...
        {
          if (!file_exists(NUKE_INCLUDE_DIR . 'utf/data/recode_basic.php'))
          {
            trigger_error('Basic reencoder file is missing', E_USER_ERROR);
          }
          include(NUKE_INCLUDE_DIR . 'utf/data/recode_basic.php');
        }
  ...
  if (preg_match('/sjis(?:[_ -]?win)?|(?:cp|ibm)[_ -]?932|shift[_ -]?jis/', $encoding))
  {
    ...
    {
      if (!file_exists(NUKE_INCLUDE_DIR . 'utf/data/recode_cjk.php'))
      {
        trigger_error('CJK reencoder file is missing', E_USER_ERROR);
      }
      include(NUKE_INCLUDE_DIR . 'utf/data/recode_cjk.php');
    }
    return sjis($string);
  }
...
function utf8_case_fold($text, $option = 'full')
{
  ...
  if (!isset($uniarray['c']))
  {
    $uniarray['c'] = include(NUKE_INCLUDE_DIR . 'utf/data/case_fold_c.php');
  }

  // only set full if we need to
  if ($option === 'full' && !isset($uniarray['f']))
  {
    $uniarray['f'] = include(NUKE_INCLUDE_DIR . 'utf/data/case_fold_f.php');
  }

  // only set simple if we need to
  if ($option !== 'full' && !isset($uniarray['s']))
  {
    $uniarray['s'] = include(NUKE_INCLUDE_DIR . 'utf/data/case_fold_s.php');
  }
