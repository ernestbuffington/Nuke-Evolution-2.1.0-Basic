        ...
        if($result = $db->sql_query($sql)) {
            ...
            if( preg_match('/^(3\.23|4\.|5\.|6\.)/', $version) ) {
                ...
                if($result = $db->sql_query($sql)) {
                    $db_size = 0;
                    $search_size = 0; // added
                    while ($rowset = $db->sql_fetchrow($result)) {
                            if( $table_prefix != '' ) {
                                if( strstr($rowset['Name'], $table_prefix) ) {
                                    $db_size += $rowset['Data_length'] + $rowset['Index_length'];
                                }
                            } else {
                                $db_size += $rowset['Data_length'] + $rowset['Index_length'];
                            }
                            // calculate the size of the search tables only
                            if ( $rowset['Name'] == SEARCH_WORD_TABLE || $rowset['Name'] == SEARCH_MATCH_TABLE ) {
                                $search_size += $rowset['Data_length'] + $rowset['Index_length'];
                            }
                    }
