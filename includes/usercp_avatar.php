...
function user_avatar_upload($mode, $avatar_mode, &$current_avatar, &$current_type, &$error, &$error_msg, $avatar_filename, $avatar_realname, $avatar_filesize, $avatar_filetype) {
    ...
    $avatar_sql  = '';
    $avatar_data = '';
    if ( ($avatar_mode == 'remote' || $avatar_mode == 'local') && preg_match('/^(http:\/\/)?([\w\-\.]+)\:?([0-9]*)\/([^ \?&=\#\"\n\r\t<]*?(\.(jpg|jpeg|gif|png)))$/', $avatar_filename, $url_ary) ) {
        if ( empty($url_ary[4]) ) {
                $error = true;
...
function display_avatar_gallery($mode, &$category, &$user_id, &$email, &$current_email, &$coppa, &$username, &$email, &$new_password, &$cur_password, &$password_confirm, &$icq, &$aim, &$msn, &$yim, &$website, &$location, &$occupation, &$interests, &$glance_show, &$signature, &$viewemail, &$notifypm, &$allow_mass_pm, &$popup_pm, &$notifyreply, &$attachsig, &$allowhtml, &$allowbbcode, &$allowsmilies, &$showavatars, &$showsignatures, &$hideonline, &$style, &$wrap, &$language, &$timezone, &$time_mode, &$dst_time_lag, &$dateformat, &$show_quickreply, &$quickreply_mode, &$user_open_quickreply, &$session_id, $xdata = false, &$rname, &$extra_info, &$newsletter, &$hide_images) {
        global $board_config, $db, $template, $lang, $images, $theme, $phpbb_root_path;

        $dir = @opendir($board_config['avatar_gallery_path']);
        $avatar_images = array();
        while( $file = @readdir($dir) ) {
            if( !is_file($board_config['avatar_gallery_path'] . '/' . $file) && !is_link($board_config['avatar_gallery_path'] . '/' . $file)  && $file != '.' && $file != '..' ) {
        ...
        $s_categories .= '</select>';
        $s_colspan = 0;
        $max_avatars = (isset($avatar_images[$category]) ? count($avatar_images[$category]) : 0);
        ...
        $params = array('coppa', 'user_id', 'username', 'email', 'current_email', 'cur_password', 'new_password', 'password_confirm', 'icq', 'aim', 'msn', 'yim', 'website', 'location', 'occupation', 'interests', 'glance_show', 'signature', 'viewemail', 'notifypm', 'allow_mass_pm', 'popup_pm', 'notifyreply', 'attachsig', 'allowhtml', 'allowbbcode', 'allowsmilies', 'showavatars', 'showsignatures', 'hideonline', 'style', 'wrap', 'language', 'timezone', 'time_mode', 'dst_time_lag', 'dateformat', 'show_quickreply', 'quickreply_mode', 'rname', 'extra_info', 'newsletter', 'hide_images', 'user_open_quickreply');
        ...
        $template->assign_vars(array(
            ...
            'S_COLSPAN'         => $s_colspan,
            'S_PROFILE_ACTION'  => (defined('IN_ADMIN_USERS') ? admin_sid('admin_users&amp;mode='.$mode) : append_sid('profile.php?mode=' . $mode)),
