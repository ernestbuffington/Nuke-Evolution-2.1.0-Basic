...
@include_once(NUKE_INCLUDE_DIR . 'functions_calendar.php');
@include_once(NUKE_INCLUDE_DIR . 'functions_announces.php');
...
function topic_list($box, $tpl='', $topic_rowset, $list_title='', $split_type=false, $display_nav_tree=true, $footer='', $inbox=true, $select_field='', $select_type=0, $select_formname='', $select_values=array()) {
    global $db, $template, $board_config, $userdata, $lang, $images, $HTTP_COOKIE_VARS, $tree;
    static $box_id;
    ...
    for ($i=0; $i < count($topic_rowset); $i++)  {
        ...
        if (defined('POST_BIRTHDAY') && ($topic_rowset[$i]['topic_type'] == POST_BIRTHDAY)) {
            ...
            $newest_post_img = '';
            if ( is_user() && ($topic_item_type == POST_TOPIC_URL) ) {
        ...
        switch ($topic_item_type) {
            case POST_USERS_URL:
                $view_topic_url   = append_sid('profile.php?' . POST_USERS_URL . '='.$topic_id);
                break;
            default:
                ...
                $last_post_author = ( $topic_rowset[$i]['id2'] == ANONYMOUS ) ? ( ($topic_rowset[$i]['post_username2'] != '' ) ? $topic_rowset[$i]['post_username2'] . ' ' : $lang['Guest'] . ' ' ) : '<a href="' . append_sid("profile.php?mode=viewprofile&amp;" . POST_USERS_URL . '='  . $topic_rowset[$i]['id2']) . '">' . UsernameColor($topic_rowset[$i]['user2']) . '</a>';
        ...
        if ( $display_nav_tree && !empty($topic_rowset[$i]['forum_id']) ) {
            if ($cat_hierarchy) {
                ...
                if ($is_auth[ $topic_rowset[$i]['forum_id'] ]['auth_view']) {
                    $nav_tree = '<a href="' . append_sid("viewforum.php?f=" . $topic_rowset[$i]['forum_id']) . '" class="gensmall">' . $topic_rowset[$i]['forum_name'] . '</a>';
                }
            }
        }
        if (!empty($nav_tree)) {
