...
/************************ End Default Options **********************/
   ...
   class PhpCaptcha {
      ...
      function SetModulName($sModulName) {
         $this->sModulName = $sModulName;
      }
      ...
      function GenerateCode() {
         ...
         if ($this->bCaseInsensitive) {
            $_SESSION[CAPTCHA_SESSION_ID.$this->sModulName] = strtoupper($this->sCode);
         } else {
            $_SESSION[CAPTCHA_SESSION_ID.$this->sModulName] = $this->sCode;
         }
      }
      ...
      function Validate($sUserCode, $bCaseInsensitive = true, $sModulName = 'phpCaptcha') {
         if ($bCaseInsensitive) {
            $sUserCode = strtoupper($sUserCode);
         }
         if (!empty($_SESSION[CAPTCHA_SESSION_ID.$sModulName]) && $sUserCode == $_SESSION[CAPTCHA_SESSION_ID.$sModulName]) {
            // clear to prevent re-use
            unset($_SESSION[CAPTCHA_SESSION_ID.$sModulName]);
            return true;
         }
         return false;
      }
   ...
   class AudioPhpCaptcha {
      ...
      function AudioPhpCaptcha(
         ...
         if (isset($_SESSION[CAPTCHA_SESSION_ID.$this->sModulName])) {
            $this->sCode = $_SESSION[CAPTCHA_SESSION_ID.$this->sModulName];
         }
      }
