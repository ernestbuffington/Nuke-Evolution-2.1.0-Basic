...
function deepPurifier($data) {
    ...
    $test = implode(' ', $data);
    if (function_exists('st_clean_string')) {
        $test = st_clean_string($test);
    } else {
          ...
          $test = str_replace($st_fr1, $st_to1, $test);
          $test = str_replace($st_fr2, $st_to2, $test);
    }
    if (!preg_match('[<|>]', $test)) {
        return $data;
    }
    ...
    if (!isset($config) || empty($config)) {
        ...
        require_once(NUKE_INCLUDE_DIR.'HTMLPurifier/HTMLPurifier/Filter/YouTube.php');
