...
class InputFilter {
    ...
    function filtered($filtered, $source = '') {
        $logdata = array(_ERROR_LOG_BLOCKED_HTML_TAG_TEXT,
                         _ERROR_LOG_BLOCKED_HTML_TAG_STRING .'&nbsp;'. htmlspecialchars($filtered),
                         _ERROR_LOG_BLOCKED_HTML_TAG_SOURCE .'&nbsp;'. htmlspecialchars($source)
                        );
        if(function_exists('log_write')) {
            log_write('error', $logdata, _ERROR_LOG_SCRIPT_ATTACK);
        } else {
            @include_once(NUKE_INCLUDE_DIR.'log.php');
            log_write('error', $logdata, _ERROR_LOG_SCRIPT_ATTACK);
        }
        echo '"' . htmlspecialchars($filtered) . '"&nbsp;'. _ERROR_LOG_BLOCKED_HTML_TAG_ECHOMSG .'<br />'. htmlspecialchars($source);
        exit;
    }
