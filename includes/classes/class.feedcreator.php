...
class FeedItem extends HtmlDescribable {
     ...
     */
    var $author, $authorEmail, $image, $category, $comments, $guid, $source, $creator, $encoding;
...
class RSSCreator10 extends FeedCreator {
    ...
    function createFeed() {
        ...
        $feed.= "<rdf:RDF\n";
        $feed.= "    xmlns=\"http://purl.org/rss/".$this->RSSVersion."/\"\n";
...
class RSSCreator091 extends FeedCreator {
    ...
    function createFeed() {
        ...
        if (isset($this->webmaster) && $this->webmaster!="") {
            $feed.= "        <webMaster>".FeedCreator::iTrunc(htmlspecialchars($this->webmaster),100)."</webMaster>\n";
        }
        if (isset($this->pubDate) && $this->pubDate!="") {
        ...
        if (isset($this->docs) && $this->docs!="") {
            $feed.= "        <docs>".FeedCreator::iTrunc(htmlspecialchars($this->docs),500)."</docs>\n";
        }
        if (isset($this->ttl) && $this->ttl!="") {
            $feed.= "        <ttl>".htmlspecialchars($this->ttl)."</ttl>\n";
        }
        if (isset($this->rating) && $this->rating!="") {
            $feed.= "        <rating>".FeedCreator::iTrunc(htmlspecialchars($this->rating),500)."</rating>\n";
        }
        if (isset($this->skipHours) && $this->skipHours!="") {
            $feed.= "        <skipHours>".htmlspecialchars($this->skipHours)."</skipHours>\n";
        }
        if (isset($this->skipDays) && $this->skipDays!="") {
 ...
 * @author Kai Blankenhorn <kaib@bitfolge.de>
 */
class RSSCreator20 extends FeedCreator {
    ...
    function createFeed() {
        $feed = "<?xml version=\"1.0\" encoding=\"".$this->encoding."\"?>\n";
//    $feed.= "<atom:link href=\"" . $rss->syndicationURL ."\" rel=\"self\" type=\"application/rss+xml\"></atom>\n";
        $feed.= $this->_createGeneratorComment();
        if (!isset($this->cssStyleSheet) || $this->cssStyleSheet=="") {
            $this->cssStyleSheet = "http://www.w3.org/2000/08/w3c-synd/style.css";
        }
        ...
        if ($this->image!=null) {
            ...
            if (isset($this->image->height) && $this->image->height!="") {
                $feed.= "            <height>".$this->image->height."</height>\n";
            }
            if (isset($this->image->description) && $this->image->description!="") {
                $feed.= "            <description>".$this->image->getDescription()."</description>\n";
            }
            $feed.= "        </image>\n";
        }
        ...
        for ($i=0;$i<count($this->items);$i++) {
            ...
            if ($this->items[$i]->author!="") {
                $feed.= "            <author>".$this->items[$i]->author."</author>\n";
            }
            /*
            // on hold
            if ($this->items[$i]->source!="") {
            ...
            */
            ...
            if ($this->items[$i]->guid!="") {
                $feed.= "            <guid isPermaLink=\"". ((isset($this->permaLink) && $this->permaLink) ? 'true' : 'false') ."\">".htmlspecialchars($this->items[$i]->guid)."</guid>\n";
            }
            $feed.= $this->_createAdditionalElements($this->items[$i]->additionalElements, "        ");
 ...
 class AtomCreator10 extends FeedCreator {
    ...
    function createFeed() {
        ...
        for ($i=0;$i<count($this->items);$i++) {
            ...
            if ($this->items[$i]->updated!="") {
                $itemUpdate = new FeedDate($this->items[$i]->updated);
                $feed.= "        <updated>".htmlspecialchars($itemUpdate->iso8601())."</updated>\n";
            }
