...
function start_edit(val, newname) {
	...
	return overlib('<img src="' + imagepath + newname + '.gif">', FULLHTML);
}
...
function change_auth(which, baseauth, forumid, type) {
	...
	if (edit_on==true) {
		if (baseauth != edit_cur_name) {
			which.src = imagepath + baseauth + '_TO_' + edit_cur_name + '.gif';
			inputEl.value = edit_cur_name;
		} else {
			which.src = imagepath + baseauth + '.gif';
			inputEl.value = '';
		}
...
function start_restore() {
	edit_on = false;
	restore_on = true;
	return overlib('<img src="' + imagepath + '_restore.gif">', FULLHTML);
}